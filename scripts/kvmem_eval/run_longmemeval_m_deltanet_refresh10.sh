...
SEED_PREFIX=${SEED_PREFIX:-lme-accumulated}
...
echo "[$(date '+%F %T')] exact-config selection + accumulated-state capture" | tee -a "$RUN_LOG"
...
"$ROOT/.venv/bin/python" \
  ...
  --state-capture-prefix "$SEED_PREFIX" --state-dir "$STATE_DIR" \
  2>&1 | tee -a "$RUN_LOG"
stop_server

echo "[$(date '+%F %T')] seeded DeltaNet refresh + final import" | tee -a "$RUN_LOG"
...
run_control() {
  "$ROOT/.venv/bin/python" \
    ...
    --baseline "$BASELINE" --seed-state-prefix "$SEED_PREFIX" \
    --expected-immutable-source-k 1 \
    --expected-block-tokens 32 --expected-budget-blocks 7168 \
    --expected-sink-blocks 8 --expected-recent-blocks 0 \
    --expected-method mean-k "$@"
