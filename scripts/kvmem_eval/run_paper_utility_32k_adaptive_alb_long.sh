...
# Both runs use the same K=32K/G=32K Adaptive Multi-Prototype policy and a
# CPU-only lower tier.  They are intentionally serialized on one GPU.
set -euo pipefail

ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)
STAMP=${STAMP:-$(date +%Y%m%d_%H%M%S)}
OUT_BASE=${OUT_BASE:-/data/chaidi/kvmem_eval/results/paper_utility_alb_long_k32g32_adaptive_cpu_only_${STAMP}}
...
run_one \
  "paper_utility_alb_512k_k32g32_adaptive_cpu_only_${STAMP}" \
  ...
  100 AgentLongBench-512K-K32G32-Adaptive-CPU-only

run_one \
  "paper_utility_alb_1m_k32g32_adaptive_cpu_only_${STAMP}" \
  ...
  50 AgentLongBench-1M-K32G32-Adaptive-CPU-only
