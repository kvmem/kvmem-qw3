...
void usage(std::ostream &os) {
    os <<
        ...
        "                        LM head. Default: off.\n"
        "  --vision-model DIR    Enable the Qwen3.5 visual frontend using DIR's\n"
        "                        HF vision weights.\n"
        "  --vision-device NAME  Visual tower device: cpu or cuda. Default: cpu.\n"
        "  --vision-cpu-model DIR  Legacy alias for --vision-model DIR\n"
        "                        --vision-device cpu.\n"
    ...
    try {
        for (int i = arg_start; i < argc; ++i) {
            ...
            if (arg == "-h" || arg == "--help") {
                ...
                engine.vision_device = "cpu";
            } else if (arg == "--vision-model") {
                engine.vision_model_path = need(arg);
            } else if (arg == "--vision-device") {
                engine.vision_device = need(arg);
                if (engine.vision_device != "cpu" &&
                    engine.vision_device != "cuda") {
                    throw std::runtime_error(
                        "invalid --vision-device (want cpu|cuda): " +
                        engine.vision_device);
                }
