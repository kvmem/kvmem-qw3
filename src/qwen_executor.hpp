...
public:
    ...
    // Host-resident counterpart used by long-lived prefix checkpoints. The
    // recurrent and conv tensors are already FP32 on device, so storing their
    // raw bytes on the host is lossless. Hidden tensors retain their native
    // dtype. One contiguous pinned allocation avoids both persistent snapshot
    // VRAM and per-request host allocation churn.
