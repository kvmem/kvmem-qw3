...
size_t kvmem_adaptive_pack_threads() {
    // CPU Adaptive streams tens of GiB of a contiguous pageable index through
    // pinned H2D slabs for every semantic query.  Four workers, which is a good
    // default for sparse KV gathering, leaves this pure memcpy stage limited to
    // roughly 15 GiB/s on the profiled GPU-local NUMA node.  Use a separately
    // tunable wider pool so index packing can approach local DRAM bandwidth
    // without also over-threading small/raw-K block gathers.
    static const size_t value =
        kvmem_env_size("QW3_KVMEM_ADAPTIVE_PACK_THREADS", 12, 1, 32);
    return value;
}
...
QwenExecutor::KvMemTierUsage QwenExecutor::kvmem_tier_usage() const {
    ...
    if (kvmem_nvme_tier_ || kvmem_raw_k_nvme_tier_) {
        u.nvme_tier = true;
        if (kvmem_nvme_tier_) {
            u.nvme_spill_bytes =
                static_cast<uint64_t>(kvmem_nvme_tier_->used_slots()) *
                u.block_bytes;
        ...
        u.nvme_used_bytes =
            u.nvme_spill_bytes + u.nvme_raw_k_bytes;
    }
    return u;
}
...
void QwenExecutor::reset_state() {
    ...
    // Packed transfer keeps the bounded host allocation topology warm across
    // independent requests. The contents are never reused as cache authority:
    // clear() and the validity maps below invalidate every block/token. Only
    // the backing allocations survive, capped by --kvmem-cpu-gb. This avoids
    // repeatedly
    ...
    const bool retain_cold_host =
        ...
        block_store_->config().optimize_pack &&
    ...
    std::fill(kvmem_raw_mtp_k_nvme_backed_.begin(),
              kvmem_raw_mtp_k_nvme_backed_.end(), uint8_t{0});
    std::fill(kvmem_raw_k_dirty_.begin(),
              kvmem_raw_k_dirty_.end(), uint8_t{0});
    std::fill(kvmem_raw_mtp_k_dirty_.begin(),
              kvmem_raw_mtp_k_dirty_.end(), uint8_t{0});
    std::fill(kvmem_raw_k_write_pending_.begin(),
              kvmem_raw_k_write_pending_.end(), uint8_t{0});
    std::fill(kvmem_raw_writeback_chunk_slot_.begin(),
              kvmem_raw_writeback_chunk_slot_.end(),
              static_cast<int8_t>(-1));
    std::fill(kvmem_raw_writeback_chunk_offset_.begin(),
              kvmem_raw_writeback_chunk_offset_.end(), uint64_t{0});
    std::fill(kvmem_raw_writeback_main_ready_.begin(),
              kvmem_raw_writeback_main_ready_.end(), uint8_t{0});
    std::fill(kvmem_raw_writeback_mtp_ready_.begin(),
              kvmem_raw_writeback_mtp_ready_.end(), uint8_t{0});
    kvmem_raw_writeback_fill_slot_ = 0;
    ...
    // here, so no raw-K authority belongs to the next request. Pack may retain
    // the bounded allocations as an invalid buffer pool; with Pack disabled,
    // their physical contents are released and allocated again on demand.
    ...
    kvmem_raw_k_debug_dump_done_ = false;
#if defined(__GLIBC__)
    ...
    g_adaptive_layer_values_pinned_.clear();
    g_adaptive_layer_values_pinned_size_.clear();
    g_adaptive_layer_values_pinned_capacity_.clear();
    ...
    kvmem_drain_adaptive_gpu_uploads();
    g_adaptive_gpu_layer_arenas_.clear();
    for (auto &done : g_adaptive_upload_done_) done.reset();
    g_adaptive_upload_sequence_ = 0;
    g_adaptive_incremental_upload_bytes_ = 0;
    g_adaptive_incremental_pack_ns_ = 0;
    g_adaptive_incremental_wait_ns_ = 0;
    ...
    g_adaptive_stream_logits_.reset();
    g_adaptive_stream_logits_capacity_ = 0;
    g_adaptive_query_head_major_.reset();
    g_adaptive_query_head_major_capacity_ = 0;
...
bool QwenExecutor::kvmem_mean_index_cpu() const {
    return block_store_ &&
           block_store_->config().index_placement ==
...
bool QwenExecutor::kvmem_mean_index_available() const {
    ...
    return kvmem_mean_index_cpu()
        ...
        : g_kbar_multi_ != nullptr;
}

void QwenExecutor::kvmem_prepare_cpu_mean_index(
    ...
    const uint32_t elem_size =
        (std::getenv("QW3_KV_DTYPE") &&
         std::strcmp(std::getenv("QW3_KV_DTYPE"), "fp32") == 0)
            ? sizeof(float)
            : sizeof(uint16_t);
    if (elements > UINT64_MAX / elem_size) {
        throw std::runtime_error("KVMem CPU mean-index byte size overflow");
    }
    const uint64_t bytes = elements * elem_size;
    if (bytes > SIZE_MAX) {
        throw std::runtime_error(
            "KVMem CPU mean-index exceeds host address space");
    }
    const bool incompatible =
        g_kbar_multi_host_elem_size_ != 0 &&
        g_kbar_multi_host_elem_size_ != elem_size;
    if (preserve_content_index &&
        (!g_kbar_multi_host_ || incompatible ||
         g_kbar_multi_host_capacity_bytes_ < bytes)) {
        throw std::runtime_error(
            "KVMem cannot preserve the CPU mean-K content index");
    }
    if (!g_kbar_multi_host_ || incompatible ||
        g_kbar_multi_host_capacity_bytes_ < bytes) {
        g_kbar_multi_host_.reset(
            bytes > 0 ? new uint8_t[static_cast<size_t>(bytes)] : nullptr);
        g_kbar_multi_host_capacity_bytes_ = bytes;
    }
    g_kbar_multi_host_bytes_ = bytes;
    g_kbar_multi_host_elem_size_ = elem_size;

    const uint64_t block_elems =
        static_cast<uint64_t>(layers) * ns *
        cfg.n_kv_heads * cfg.head_dim;
    if (block_elems == 0 ||
        block_elems > UINT64_MAX / elem_size) {
        throw std::runtime_error(
            "KVMem CPU mean-index staging shape overflow");
    }
    const uint64_t bytes_per_block = block_elems * elem_size;
    const uint64_t requested =
        std::max<uint64_t>(
            bytes_per_block,
            block_store_->config().index_staging_bytes);
    uint64_t tile_blocks64 = requested / bytes_per_block;
    // Pass-1 stores one tile's logits in dynamic shared memory.
    tile_blocks64 = std::min<uint64_t>(
        tile_blocks64, std::max<uint64_t>(1, 8192u / ns));
    tile_blocks64 = std::max<uint64_t>(1, tile_blocks64);
    tile_blocks64 =
        std::min<uint64_t>(tile_blocks64, stride_blocks);
    if (tile_blocks64 > UINT32_MAX) {
        throw std::runtime_error(
            "KVMem CPU mean-index staging block count overflow");
    }
    const uint32_t tile_blocks =
        static_cast<uint32_t>(tile_blocks64);
    const uint64_t slot_elems = block_elems * tile_blocks;
    const uint64_t slot_bytes = slot_elems * elem_size;
    const bool rebuild_slots =
        g_kbar_stream_tile_blocks_ != tile_blocks ||
        g_kbar_stream_slot_bytes_ != slot_bytes ||
        !g_kbar_stream_slots_[0].device ||
        !g_kbar_stream_slots_[1].device;
    if (rebuild_slots) {
        for (auto &slot : g_kbar_stream_slots_) {
            if (slot.transfer_done) {
                require_status(
                    backend_.wait_kv_transfer_fence(slot.transfer_done));
            }
            slot.compute_done.reset();
            slot.capture_pending = false;
            slot.device =
                kvmem_alloc_mean_index_tensor(
                    slot_elems, "g_kbar_stream_stage");
            slot.host =
                backend_.host_buffer(
                    slot_bytes, "g_kbar_stream_host");
            if (!slot.host || slot.host->bytes < slot_bytes) {
                throw std::runtime_error(
                    "KVMem CPU mean-index host staging allocation failed");
            }
        }
        g_kbar_stream_tile_blocks_ = tile_blocks;
        g_kbar_stream_slot_bytes_ = slot_bytes;
        g_kbar_capture_next_slot_ = 0;
        g_kbar_capture_active_slot_ = -1;
    }
    if (std::getenv("QW3_KVMEM_TRACE")) {
        std::fprintf(
            stderr,
            "[bs-kbar-index] placement=cpu dtype=%s layers=%u "
            "stride_blocks=%u subblocks=%u host_gib=%.3f "
            "tile_blocks=%u slot_mib=%.2f slots=2\n",
            elem_size == sizeof(float) ? "fp32" : "fp16",
            layers, stride_blocks, ns,
...
void QwenExecutor::kvmem_drain_mean_index_capture_slot(uint32_t slot_index) {
    if (slot_index >= g_kbar_stream_slots_.size()) return;
    MeanIndexStageSlot &slot = g_kbar_stream_slots_[slot_index];
    if (!slot.capture_pending) return;
    require_status(
        backend_.wait_kv_transfer_fence(slot.transfer_done));
    if (!g_kbar_multi_host_ || !slot.host ||
        g_kbar_multi_host_elem_size_ == 0 || !block_store_) {
        throw std::runtime_error(
            "KVMem CPU mean-index capture destination unavailable");
    }
    const QwenConfig &cfg = model_.config();
    const uint32_t layers =
        std::max<uint32_t>(1, kvmem_qc_num_layers_);
    const uint32_t ns =
        std::max<uint32_t>(1, kvmem_qc_n_subblocks_);
    const uint64_t per_block_bytes =
        static_cast<uint64_t>(ns) * cfg.n_kv_heads *
        cfg.head_dim * g_kbar_multi_host_elem_size_;
    const uint64_t layer_bytes =
        static_cast<uint64_t>(slot.capture_layer_stride) *
        per_block_bytes;
    const auto *src =
        static_cast<const uint8_t *>(slot.host->data);
    for (uint32_t layer = 0; layer < layers; ++layer) {
        const uint64_t dst_block =
            static_cast<uint64_t>(layer) *
                kvmem_qc_layer_stride_blocks_ +
            slot.capture_first_block;
        const uint64_t dst_byte = dst_block * per_block_bytes;
        const uint64_t src_byte =
            static_cast<uint64_t>(layer) * layer_bytes;
        std::memcpy(
            g_kbar_multi_host_.get() + dst_byte,
            src + src_byte,
            static_cast<size_t>(
                static_cast<uint64_t>(slot.capture_blocks) *
                per_block_bytes));
    }
    slot.capture_pending = false;
    slot.capture_first_block = 0;
    slot.capture_blocks = 0;
    slot.capture_layer_stride = 0;
    slot.capture_bytes = 0;
}

void QwenExecutor::kvmem_drain_mean_index_capture() {
    for (uint32_t i = 0; i < g_kbar_stream_slots_.size(); ++i) {
        kvmem_drain_mean_index_capture_slot(i);
    }
    g_kbar_capture_active_slot_ = -1;
}

uint32_t QwenExecutor::kvmem_begin_mean_index_capture(
        uint32_t first_block, uint32_t n_blocks, bool seed_partial) {
    if (!kvmem_mean_index_cpu() || !kvmem_mean_index_available() ||
        n_blocks == 0 || n_blocks > g_kbar_stream_tile_blocks_) {
        throw std::runtime_error(
            "KVMem CPU mean-index capture exceeds fixed staging");
    }
    const uint32_t slot_index =
        g_kbar_capture_next_slot_++ %
        static_cast<uint32_t>(g_kbar_stream_slots_.size());
    kvmem_drain_mean_index_capture_slot(slot_index);
    MeanIndexStageSlot &slot = g_kbar_stream_slots_[slot_index];
    if (slot.transfer_done) {
        require_status(
            backend_.wait_kv_transfer_fence(slot.transfer_done));
    ...
    const uint64_t per_block_bytes =
        ...
        cfg.head_dim * g_kbar_multi_host_elem_size_;
    const uint64_t layer_bytes =
        static_cast<uint64_t>(n_blocks) * per_block_bytes;
    const uint64_t bytes =
        static_cast<uint64_t>(layers) * layer_bytes;
    if (seed_partial) {
        std::memset(slot.host->data, 0, static_cast<size_t>(bytes));
        auto *dst = static_cast<uint8_t *>(slot.host->data);
        for (uint32_t layer = 0; layer < layers; ++layer) {
            const uint64_t src_block =
                ...
                first_block;
            std::memcpy(
                dst + static_cast<uint64_t>(layer) * layer_bytes,
                g_kbar_multi_host_.get() +
                    src_block * per_block_bytes,
                static_cast<size_t>(per_block_bytes));
        }
        require_status(
        ...
        require_status(backend_.copy_bytes_from_host_async(
            *slot.device, 0, slot.host->data, bytes));
        require_status(
            backend_.record_kv_transfer_fence(slot.transfer_done));
        require_status(
            backend_.execution_wait_for_kv_transfer(
                slot.transfer_done));
    }
    g_kbar_capture_active_slot_ =
        static_cast<int32_t>(slot_index);
    return slot_index;
}

void QwenExecutor::kvmem_finish_mean_index_capture(
        uint32_t slot_index, uint32_t first_block,
        uint32_t n_blocks) {
    if (slot_index >= g_kbar_stream_slots_.size()) {
        throw std::runtime_error(
            "KVMem CPU mean-index capture slot out of range");
    }
    MeanIndexStageSlot &slot = g_kbar_stream_slots_[slot_index];
    const QwenConfig &cfg = model_.config();
    const uint32_t layers =
    ...
    const uint32_t ns =
        std::max<uint32_t>(1, kvmem_qc_n_subblocks_);
    const uint64_t bytes =
        static_cast<uint64_t>(layers) * n_blocks * ns *
        cfg.n_kv_heads * cfg.head_dim *
        g_kbar_multi_host_elem_size_;
    require_status(backend_.begin_kv_transfer_from_device());
    require_status(backend_.copy_bytes_to_host_async(
        *slot.device, slot.host->data, 0, bytes));
    require_status(
        backend_.record_kv_transfer_fence(slot.transfer_done));
    slot.capture_pending = true;
    slot.capture_first_block = first_block;
    slot.capture_blocks = n_blocks;
    slot.capture_layer_stride = n_blocks;
    slot.capture_bytes = bytes;
    g_kbar_capture_active_slot_ = -1;
}

void QwenExecutor::kvmem_pack_mean_index_tile(
        uint32_t first_block, uint32_t n_blocks,
        MeanIndexStageSlot &slot) {
    if (!g_kbar_multi_host_ || !slot.host ||
        n_blocks == 0 ||
        n_blocks > g_kbar_stream_tile_blocks_) {
        throw std::runtime_error(
            "KVMem CPU mean-index tile pack is unavailable");
    }
    const QwenConfig &cfg = model_.config();
    const uint32_t layers =
        std::max<uint32_t>(1, kvmem_qc_num_layers_);
...
void QwenExecutor::kvmem_evict_cpu_for_raw(uint64_t bytes) {
    ...
    while (!kvmem_cpu_budget_has(bytes)) {
        // Pack can retain empty spill slabs across cold requests. They are
        ...
        if (!kvmem_nvme_tier_ || victim_i < 0) {
            ...
            throw std::runtime_error(
                ...
                "the CPU budget or enable --kvmem-raw-k-nvme");
        }
        ...
        const bool inclusive =
            block_store_->config().optimize_stage_out;
...
bool QwenExecutor::kvmem_raw_k_chunk_complete(
        uint32_t chunk, bool mtp) const {
    const auto &valid =
        mtp ? kvmem_raw_mtp_k_valid_tokens_
            : kvmem_raw_k_valid_tokens_;
    if (valid.empty() || kvmem_raw_k_chunk_tokens_ == 0) return false;
    const uint64_t begin64 =
        static_cast<uint64_t>(chunk) * kvmem_raw_k_chunk_tokens_;
    if (begin64 >= valid.size()) return false;
    const size_t begin = static_cast<size_t>(begin64);
    const size_t end = std::min<size_t>(
        valid.size(), begin + kvmem_raw_k_chunk_tokens_);
    return std::all_of(
        valid.begin() + static_cast<std::ptrdiff_t>(begin),
        valid.begin() + static_cast<std::ptrdiff_t>(end),
        [](uint8_t v) { return v != 0; });
}

void QwenExecutor::kvmem_persist_raw_k_chunk(
        uint32_t chunk, bool mtp) {
    ...
    auto &dirty =
        mtp ? kvmem_raw_mtp_k_dirty_ : kvmem_raw_k_dirty_;
    const uint64_t bytes =
        mtp ? kvmem_raw_mtp_k_chunk_bytes_
            : kvmem_raw_k_chunk_bytes_;
    if (chunk >= chunks.size() || chunk >= backed.size() ||
        chunk >= dirty.size() || bytes == 0) {
        throw std::runtime_error(
            "KVMem raw-K NVMe chunk metadata is out of range");
    }
    kvmem_wait_raw_k_chunk_write(chunk);
    if (backed[chunk] && !dirty[chunk]) return;
    if (!chunks[chunk]) {
        throw std::runtime_error(
    ...
    kvmem_raw_k_nvme_tier_->write_slot_range(
        placement.slot, segment_offset, chunks[chunk].get(), bytes);
    backed[chunk] = 1;
    dirty[chunk] = 0;
    kvmem_raw_k_nvme_write_bytes_ += bytes;
    if (kvmem_tier_trace_enabled()) {
        std::fprintf(
            stderr,
            "[kvmem-raw-k-nvme] op=write chunk=%u segment=%s "
            "slot=%d bytes=%llu cumulative_write_bytes=%llu\n",
            chunk, mtp ? "mtp" : "main", placement.slot,
...
void QwenExecutor::kvmem_reap_raw_k_writes(bool wait_all) {
    ...
    for (uint32_t slot_index = 0;
        ...
        const NvmeBatchIoStats stats = slot.future.get();
        kvmem_raw_k_nvme_write_bytes_ += stats.bytes;
        kvmem_raw_k_nvme_write_syscalls_ += stats.syscalls;
        kvmem_raw_k_nvme_write_ns_ += stats.duration_ns;
        for (uint32_t chunk : slot.chunks) {
            const uint64_t record_offset =
                chunk < kvmem_raw_writeback_chunk_offset_.size()
                    ? kvmem_raw_writeback_chunk_offset_[chunk]
                    : 0;
            const uint8_t *record =
                static_cast<const uint8_t *>(slot.host->data) +
                record_offset;
            // Direct full-chunk D2H intentionally leaves the pageable cache
            // unpublished while the GPU and SSD pipelines own this staging
            ...
            if (chunk < kvmem_raw_mtp_k_dirty_.size()) {
                kvmem_raw_mtp_k_dirty_[chunk] = 0;
            }
            if (chunk < kvmem_raw_k_write_pending_.size()) {
                kvmem_raw_k_write_pending_[chunk] = 0;
            }
            if (chunk < kvmem_raw_writeback_chunk_slot_.size() &&
                kvmem_raw_writeback_chunk_slot_[chunk] ==
                    static_cast<int8_t>(slot_index)) {
                kvmem_raw_writeback_chunk_slot_[chunk] = -1;
                kvmem_raw_writeback_chunk_offset_[chunk] = 0;
                kvmem_raw_writeback_main_ready_[chunk] = 0;
                kvmem_raw_writeback_mtp_ready_[chunk] = 0;
            }
        }
        if (kvmem_tier_trace_enabled()) {
            std::fprintf(
                stderr,
                "[kvmem-raw-k-nvme] op=write-batch slot=%u "
                "chunks=%zu bytes=%llu syscalls=%llu io_ms=%.3f "
                "cumulative_write_bytes=%llu\n",
                slot_index, slot.chunks.size(),
...
void QwenExecutor::kvmem_submit_raw_k_writeback_slot(
        uint32_t slot_index) {
    if (slot_index >= kvmem_raw_writeback_slots_.size()) {
        throw std::runtime_error(
            "KVMem raw-K writeback slot is out of range");
    }
    RawWritebackSlot &slot =
        kvmem_raw_writeback_slots_[slot_index];
    ...
    NvmeKvTier *tier = kvmem_raw_k_nvme_tier_.get();
    require_status(
        backend_.record_kv_transfer_fence(slot.d2h_done));
    DeviceBackend *backend = &backend_;
    void *data = slot.host->data;
    const uint64_t buffer_bytes = slot.used_bytes;
    const std::vector<NvmeIoSpan> spans = slot.spans;
    std::unique_ptr<DeviceTransferFence> d2h_done =
        std::move(slot.d2h_done);
    slot.submit_ns = kvmem_steady_ns();
    slot.in_flight = true;
    slot.future = std::async(
        std::launch::async,
        [tier, backend, data, buffer_bytes, spans,
         d2h_done = std::move(d2h_done)]() mutable {
            const DeviceStatus transfer_status =
                backend->wait_kv_transfer_fence(d2h_done);
            if (!transfer_status.ok) {
                throw std::runtime_error(
                    std::string(
                        "KVMem raw-K D2H fence wait failed: ") +
                    transfer_status.message);
            }
            NvmeBatchIoStats stats;
            const uint64_t start_ns = kvmem_steady_ns();
            tier->write_spans(
                spans, data, buffer_bytes, &stats);
            stats.duration_ns =
                kvmem_steady_ns() - start_ns;
            return stats;
        });
}

bool QwenExecutor::kvmem_reserve_raw_k_writeback_record(
        uint32_t chunk, uint32_t &slot_index,
        uint64_t &record_offset) {
    if (!kvmem_raw_k_nvme_enabled_ ||
        kvmem_raw_writeback_record_bytes_ == 0 ||
        chunk >= kvmem_raw_writeback_chunk_slot_.size()) {
        return false;
    }
    const int8_t existing =
        kvmem_raw_writeback_chunk_slot_[chunk];
    if (existing >= 0) {
        slot_index = static_cast<uint32_t>(existing);
        record_offset =
            kvmem_raw_writeback_chunk_offset_[chunk];
        return true;
    }
    for (;;) {
        kvmem_reap_raw_k_writes(/*wait_all=*/false);
        RawWritebackSlot &slot =
            kvmem_raw_writeback_slots_[
                kvmem_raw_writeback_fill_slot_];
        if (!slot.in_flight && slot.host &&
            slot.used_bytes + kvmem_raw_writeback_record_bytes_ <=
                slot.capacity_bytes) {
            slot_index = kvmem_raw_writeback_fill_slot_;
            record_offset = slot.used_bytes;
            slot.used_bytes +=
        ...
        if (!slot.in_flight && slot.used_bytes > 0) {
            kvmem_submit_raw_k_writeback_slot(
                kvmem_raw_writeback_fill_slot_);
        }
        kvmem_raw_writeback_fill_slot_ =
            (kvmem_raw_writeback_fill_slot_ + 1u) %
            static_cast<uint32_t>(
                kvmem_raw_writeback_slots_.size());
        RawWritebackSlot &next =
            kvmem_raw_writeback_slots_[
                kvmem_raw_writeback_fill_slot_];
        if (next.in_flight) {
            next.future.wait();
...
bool QwenExecutor::kvmem_copy_raw_k_chunk_to_writeback(
        uint32_t chunk) {
    if (!kvmem_raw_k_nvme_enabled_ ||
        chunk >= kvmem_raw_k_chunks_.size()) {
        return false;
    }
    const bool main_complete =
        kvmem_raw_k_chunk_complete(chunk, /*mtp=*/false);
    const bool mtp_complete =
        kvmem_raw_mtp_k_chunk_bytes_ == 0 ||
        kvmem_raw_k_chunk_complete(chunk, /*mtp=*/true);
    if (!main_complete || !mtp_complete) return false;
    if (chunk < kvmem_raw_k_write_pending_.size() &&
        kvmem_raw_k_write_pending_[chunk]) {
        return true;
    }
    const bool main_clean =
        chunk < kvmem_raw_k_nvme_backed_.size() &&
        kvmem_raw_k_nvme_backed_[chunk] &&
        chunk < kvmem_raw_k_dirty_.size() &&
        !kvmem_raw_k_dirty_[chunk];
    const bool mtp_clean =
        kvmem_raw_mtp_k_chunk_bytes_ == 0 ||
        (chunk < kvmem_raw_mtp_k_nvme_backed_.size() &&
         kvmem_raw_mtp_k_nvme_backed_[chunk] &&
         chunk < kvmem_raw_mtp_k_dirty_.size() &&
         !kvmem_raw_mtp_k_dirty_[chunk]);
    if (main_clean && mtp_clean) return true;

    uint32_t slot_index = 0;
    uint64_t record_offset = 0;
    if (!kvmem_reserve_raw_k_writeback_record(
            chunk, slot_index, record_offset)) {
        return false;
    }
    RawWritebackSlot &slot =
        kvmem_raw_writeback_slots_[slot_index];
...
void QwenExecutor::kvmem_flush_raw_k_writes(bool wait_all) {
    if (!kvmem_raw_k_nvme_enabled_) return;
    for (uint32_t i = 0;
         i < kvmem_raw_writeback_slots_.size(); ++i) {
        RawWritebackSlot &slot =
            kvmem_raw_writeback_slots_[i];
        if (!slot.in_flight && slot.used_bytes > 0) {
            kvmem_submit_raw_k_writeback_slot(i);
        }
    }
    kvmem_reap_raw_k_writes(wait_all);
}

void QwenExecutor::kvmem_wait_raw_k_chunk_write(
        uint32_t chunk) {
    if (chunk >= kvmem_raw_k_write_pending_.size() ||
        !kvmem_raw_k_write_pending_[chunk]) {
        return;
...
void QwenExecutor::kvmem_sync_raw_k_persistence() {
    kvmem_flush_raw_k_writes(/*wait_all=*/true);
}
...
void QwenExecutor::kvmem_set_archive(KvMemArchive *archive, bool build,
                                     uint64_t checkpoint_interval_tokens) {
    kvmem_archive_ = archive;
    kvmem_archive_build_ = build;
    kvmem_archive_ckpt_interval_ = checkpoint_interval_tokens;
    kvmem_archive_next_ckpt_ = checkpoint_interval_tokens;
}

uint64_t QwenExecutor::kvmem_archive_sealable_tokens() const {
    if (!kvmem_archive_ || kvmem_raw_k_chunk_tokens_ == 0) return 0;
    // A raw-K chunk is written only once every token in it has been captured,
    // so a trailing partial chunk can never be part of a durable archive.
    const uint64_t stride = kvmem_raw_k_chunk_tokens_;
    return (static_cast<uint64_t>(kvmem_registered_pos_) / stride) * stride;
}

void QwenExecutor::kvmem_archive_persist_through(uint64_t tokens) {
    if (!kvmem_archive_building() || !block_store_) return;
    const uint32_t bt = block_store_->config().block_tokens;
    if (bt == 0 || tokens == 0) return;

    // V for blocks that never left the GPU is pushed out first; the proactive
    // writeback path already knows how to do this without disturbing residency.
    kvmem_prefill_writeback(static_cast<uint32_t>(
        std::min<uint64_t>(tokens, kvmem_registered_pos_)));
    kvmem_finish_proactive_d2h(/*wait_all=*/true);
    kvmem_reap_pending_writes(/*wait_all=*/true);

    // Anything the proactive path could not cover (for example a block that was
    // evicted to CPU before the archive was armed) is staged out explicitly.
    const uint32_t need_blocks = static_cast<uint32_t>((tokens + bt - 1) / bt);
    std::vector<uint32_t> force_out;
    for (const KvMemBlock &b : block_store_->blocks()) {
        if (b.block_id >= need_blocks) break;
        if (b.ssd_clean && b.nvme_slot >= 0) continue;
        force_out.push_back(b.block_id);
    }
    if (!force_out.empty()) {
        kvmem_stage_out(force_out);
        kvmem_finish_proactive_d2h(/*wait_all=*/true);
        kvmem_reap_pending_writes(/*wait_all=*/true);
    }
    ...
    // Record what is actually durable. Deriving the bitmaps from live tier
    // state rather than hooking every write site keeps the two in step even
    // when a block takes an unusual route to the arena.
    const uint32_t need_chunks = static_cast<uint32_t>(
        (tokens + kvmem_raw_k_chunk_tokens_ - 1) / kvmem_raw_k_chunk_tokens_);
    kvmem_archive_->reserve_chunks(need_chunks);
    kvmem_archive_->reserve_blocks(need_blocks);
    for (uint32_t c = 0; c < need_chunks; ++c) {
        if (c < kvmem_raw_k_nvme_backed_.size() &&
            kvmem_raw_k_nvme_backed_[c] && !kvmem_raw_k_dirty_[c]) {
            kvmem_archive_->mark_chunk_valid(c);
        }
    }
    for (const KvMemBlock &b : block_store_->blocks()) {
        if (b.block_id >= need_blocks) break;
        if (b.ssd_clean && b.nvme_slot == static_cast<int32_t>(b.block_id)) {
            kvmem_archive_->mark_block_valid(b.block_id);
        }
    }
}

void QwenExecutor::kvmem_archive_capture_ladder(uint64_t position) {
    if (!kvmem_archive_building()) return;
    ensure_scratch();
    KvMemArchiveState state;
    state.position = position;
    state.kvmem_registered_pos = kvmem_registered_pos_;
    state.mtp_prefix_len = mtp_prefix_len_;

    // DeltaNet state is strictly sequential, so it is the one thing a
    // truncation or a resumed build cannot recompute cheaply. Everything else
    // in the archive is addressable by position.
    auto add = [&](uint32_t kind, uint32_t index, const DeviceTensor *t) {
        if (!t || t->count == 0) return;
        const uint64_t bytes = t->count * t->elem_size;
        const size_t base = state.payload.size();
        state.payload.resize(base + static_cast<size_t>(bytes));
        require_status(backend_.copy_bytes_to_host(
            *t, state.payload.data() + base, /*byte_offset=*/0, bytes));
        state.entries.push_back({kind, index, bytes});
    };
    add(KvMemArchiveStateEntry::Hidden, 0, h_.get());
    add(KvMemArchiveStateEntry::MtpPrefixHidden, 0, mtp_prefix_h_.get());
    for (size_t i = 0; i < recurrent_states_.size(); ++i) {
        add(KvMemArchiveStateEntry::Recurrent, static_cast<uint32_t>(i),
            recurrent_states_[i].get());
    }
    for (size_t i = 0; i < conv_states_.size(); ++i) {
        add(KvMemArchiveStateEntry::Conv, static_cast<uint32_t>(i),
            conv_states_[i].get());
    }
    kvmem_archive_->write_state(state);
}

void QwenExecutor::kvmem_archive_restore_ladder(uint64_t position) {
    if (!kvmem_archive_) {
        throw std::runtime_error("no KVMem context archive is attached");
    }
    ensure_scratch();
    // `ensure_scratch()` allocates the main hidden/recurrent state but not the
    // speculative MTP state.  A cold archive attach therefore used to see a
    // null `mtp_prefix_h_` below and silently skip the persisted
    // MtpPrefixHidden entry.  Only the first MTP row after a resume boundary
    // was then built from an uninitialised prefix; the first resumed batch
    // overwrote the prefix again, which made every later row look correct.
    // Allocate the MTP state before interpreting the ladder so every archived
    // entry has a concrete restore target.
    ensure_mtp_scratch();
    const KvMemArchiveState state = kvmem_archive_->read_state(position);
    uint64_t offset = 0;
    for (const KvMemArchiveStateEntry &e : state.entries) {
        DeviceTensor *dst = nullptr;
        switch (e.kind) {
            case KvMemArchiveStateEntry::Hidden: dst = h_.get(); break;
            case KvMemArchiveStateEntry::MtpPrefixHidden:
                dst = mtp_prefix_h_.get();
                break;
            case KvMemArchiveStateEntry::Recurrent:
                if (e.index < recurrent_states_.size()) {
                    dst = recurrent_states_[e.index].get();
                }
                break;
            case KvMemArchiveStateEntry::Conv:
                if (e.index < conv_states_.size()) {
                    dst = conv_states_[e.index].get();
                }
                break;
            default: break;
        }
        if (dst == nullptr) {
            throw std::runtime_error(
                "KVMem archive ladder state has no runtime restore target for "
                "entry kind " + std::to_string(e.kind) + " index " +
                std::to_string(e.index));
        }
        if (dst->count * dst->elem_size != e.bytes) {
            throw std::runtime_error(
                "KVMem archive ladder state has an unexpected tensor size; "
                "the archive layout does not match this engine");
        }
        require_status(backend_.copy_bytes_from_host(
            *dst, /*byte_offset=*/0, state.payload.data() + offset,
            e.bytes));
        offset += e.bytes;
    }
    mtp_prefix_len_ = state.mtp_prefix_len;
}

void QwenExecutor::kvmem_persist_completed_raw_k_chunks(
        uint32_t base, uint32_t rows, bool mtp) {
    if (!kvmem_raw_k_nvme_enabled_ || rows == 0) return;
    ...
    const uint32_t last = static_cast<uint32_t>(
        (end - 1) / kvmem_raw_k_chunk_tokens_);
    for (uint32_t chunk = first; chunk <= last; ++chunk) {
        (void) mtp;
        (void) kvmem_copy_raw_k_chunk_to_writeback(chunk);
    }
}
...
bool QwenExecutor::kvmem_evict_one_raw_k_chunk() {
    ...
    const uint32_t active_chunk =
        kvmem_raw_k_chunk_tokens_ == 0
            ? UINT32_MAX
            : std::min<uint32_t>(
                  static_cast<uint32_t>(
                      kvmem_raw_k_chunks_.empty()
                          ? 0 : kvmem_raw_k_chunks_.size() - 1),
                  position_ / kvmem_raw_k_chunk_tokens_);
    auto evict_from = [&](auto &chunks, auto &backed, auto &dirty,
                          const auto &valid, uint64_t chunk_bytes,
                          bool mtp) -> bool {
        if (chunk_bytes == 0) return false;
        uint32_t victim = UINT32_MAX;
        // Prefer a clean already-backed chunk, then the oldest remaining
        // resident chunk. Avoid the actively growing tail unless it is the only
        ...
        for (uint32_t pass = 0; pass < 3 && victim == UINT32_MAX; ++pass) {
            for (uint32_t chunk = 0; chunk < chunks.size(); ++chunk) {
                ...
                // persistable until that slot's transfer fence completes.
                if (chunk <
                        kvmem_raw_writeback_chunk_slot_.size() &&
                    kvmem_raw_writeback_chunk_slot_[chunk] >= 0) {
                    continue;
                }
                if (pass < 2 && chunk == active_chunk) continue;
                if (pass == 0 &&
                    !(chunk < backed.size() && backed[chunk] &&
                      chunk < dirty.size() && !dirty[chunk])) {
                    continue;
                }
                victim = chunk;
                break;
        ...
        const size_t valid_begin = std::min<size_t>(
            valid.size(),
            static_cast<size_t>(victim) *
                kvmem_raw_k_chunk_tokens_);
        const size_t valid_end = std::min<size_t>(
            valid.size(), valid_begin + kvmem_raw_k_chunk_tokens_);
        const bool has_valid = std::any_of(
            valid.begin() + static_cast<std::ptrdiff_t>(valid_begin),
            valid.begin() + static_cast<std::ptrdiff_t>(valid_end),
            [](uint8_t v) { return v != 0; });
        if (has_valid &&
            (!backed[victim] || dirty[victim])) {
            kvmem_persist_raw_k_chunk(victim, mtp);
        }
        chunks[victim].reset();
        uint64_t &resident_bytes =
            mtp ? kvmem_raw_mtp_k_mirror_bytes_
                : kvmem_raw_k_mirror_bytes_;
        resident_bytes =
            chunk_bytes <= resident_bytes
                ? resident_bytes - chunk_bytes : 0;
        if (kvmem_tier_trace_enabled()) {
            std::fprintf(
                stderr,
                "[kvmem-raw-k-nvme] op=evict chunk=%u segment=%s "
                "bytes=%llu cpu_raw_bytes=%llu\n",
                victim, mtp ? "mtp" : "main",
    ...
    if (evict_from(
            kvmem_raw_k_chunks_, kvmem_raw_k_nvme_backed_,
            kvmem_raw_k_dirty_, kvmem_raw_k_valid_tokens_,
            kvmem_raw_k_chunk_bytes_, /*mtp=*/false)) {
        return true;
    }
    return evict_from(
        kvmem_raw_mtp_k_chunks_, kvmem_raw_mtp_k_nvme_backed_,
        kvmem_raw_mtp_k_dirty_, kvmem_raw_mtp_k_valid_tokens_,
        kvmem_raw_mtp_k_chunk_bytes_, /*mtp=*/true);
}
...
void QwenExecutor::kvmem_ensure_raw_k_chunks(
    ...
    for (uint32_t chunk = first; chunk <= last; ++chunk) {
        ...
        if (!kvmem_cpu_budget_has(chunk_bytes) &&
            kvmem_raw_k_nvme_enabled_) {
            // Prefer dropping older clean raw-cache chunks first. That can
            // preserve the current partially-filled staging batch and retain
        ...
        if (!kvmem_cpu_budget_has(chunk_bytes) &&
            kvmem_raw_k_nvme_enabled_) {
            // CPU pressure is also a staging-reuse boundary. Publish any
            // complete partial batch, wait for its D2H+SSD future, copy the
            // finished records into the pageable cache, then retry eviction.
            // Direct full-chunk capture preallocates both main and MTP cache
            // segments before reserving staging, so this cannot observe the
...
void QwenExecutor::kvmem_write_raw_k(
    ...
    if (kvmem_raw_k_nvme_enabled_) {
        ...
        const uint32_t first =
            base / kvmem_raw_k_chunk_tokens_;
        const uint32_t last = static_cast<uint32_t>(
            (end - 1) / kvmem_raw_k_chunk_tokens_);
        for (uint32_t chunk = first;
             chunk <= last; ++chunk) {
            kvmem_wait_raw_k_chunk_write(chunk);
        }
    }
    ...
    while (left > 0) {
        ...
        if (chunk < dirty.size()) dirty[chunk] = 1;
        src += bytes;
        pos += take;
        left -= take;
    }
...
void QwenExecutor::kvmem_read_raw_k_block(
        uint32_t base, uint32_t rows, uint8_t *dst, bool mtp) {
    if (rows == 0 || !dst || !block_store_) return;
    const uint32_t bt = std::max<uint32_t>(
        1, block_store_->config().block_tokens);
    if (rows > bt || base % bt != 0) {
        throw std::runtime_error(
            "KVMem raw-K block read has an invalid token range");
    }
    const uint32_t chunk = base / kvmem_raw_k_chunk_tokens_;
    const uint32_t within = base % kvmem_raw_k_chunk_tokens_;
    const uint64_t layers =
        mtp ? 1 : kvmem_raw_layers_.size();
    const uint64_t block_bytes =
        static_cast<uint64_t>(layers) * bt *
    ...
    if (kvmem_raw_k_block_major_ &&
        ...
        const uint64_t byte_offset =
            block_in_chunk * block_bytes;
        if (chunks[chunk]) {
            std::memcpy(
                dst, chunks[chunk].get() + byte_offset,
    ...
    const uint64_t layer_block_bytes =
        static_cast<uint64_t>(bt) * kvmem_raw_k_row_bytes_;
    for (uint32_t layer = 0; layer < layers; ++layer) {
        uint8_t *layer_dst =
            dst + static_cast<uint64_t>(layer) * layer_block_bytes;
        kvmem_read_raw_k(layer, base, layer_dst, rows, mtp);
        const uint64_t valid_bytes =
            static_cast<uint64_t>(rows) * kvmem_raw_k_row_bytes_;
        if (valid_bytes < layer_block_bytes) {
            std::memset(
                layer_dst + valid_bytes, 0,
                static_cast<size_t>(
                    layer_block_bytes - valid_bytes));
        }
    }
}

void QwenExecutor::kvmem_maybe_dump_raw_k_window() {
    if (kvmem_raw_k_debug_dump_done_ || !kvmem_immutable_source_k_) return;
    const char *dump_path = std::getenv("QW3_KVMEM_DUMP_RAW_K");
    if (!dump_path || !*dump_path) return;
    const char *begin_env = std::getenv("QW3_KVMEM_DUMP_RAW_K_BEGIN");
    const char *end_env = std::getenv("QW3_KVMEM_DUMP_RAW_K_END");
    if (!begin_env || !end_env) {
        throw std::runtime_error(
            "QW3_KVMEM_DUMP_RAW_K requires BEGIN and END token positions");
    }
    const uint64_t begin64 = std::strtoull(begin_env, nullptr, 10);
    const uint64_t end64 = std::strtoull(end_env, nullptr, 10);
    if (end64 <= begin64 || end64 - begin64 > 4096 ||
        end64 > kvmem_raw_k_valid_tokens_.size() ||
        end64 > std::numeric_limits<uint32_t>::max()) {
        throw std::runtime_error(
            "QW3_KVMEM_DUMP_RAW_K token range is invalid or exceeds 4096");
    }
    const uint32_t begin = static_cast<uint32_t>(begin64);
    const uint32_t end = static_cast<uint32_t>(end64);
    for (uint32_t token = begin; token < end; ++token) {
        if (!kvmem_raw_k_valid_tokens_[token]) return;
    }

    const char *kv_dtype = std::getenv("QW3_KV_DTYPE");
    const bool fp32 = kv_dtype && std::strcmp(kv_dtype, "fp32") == 0;
    const bool fp8 = kv_dtype && std::strcmp(kv_dtype, "fp8") == 0;
    if (fp8) {
        throw std::runtime_error(
            "raw-K direction dump currently requires fp16 or fp32 KV");
    }
    const QwenConfig &cfg = model_.config();
    const uint32_t rows = end - begin;
    const uint64_t layer_bytes =
        static_cast<uint64_t>(rows) * kvmem_raw_k_row_bytes_;
    if (layer_bytes > SIZE_MAX ||
        kvmem_raw_layers_.size() > SIZE_MAX / layer_bytes) {
        throw std::runtime_error("raw-K direction dump size overflow");
    }
    std::vector<uint8_t> data(
        static_cast<size_t>(layer_bytes * kvmem_raw_layers_.size()));
    for (uint32_t slot = 0; slot < kvmem_raw_layers_.size(); ++slot) {
        kvmem_read_raw_k(
            slot, begin,
            data.data() + static_cast<uint64_t>(slot) * layer_bytes,
            rows, /*mtp=*/false);
    }

    {
        std::ofstream binary(dump_path, std::ios::binary | std::ios::trunc);
        if (!binary) {
            throw std::runtime_error(
                std::string("could not open raw-K dump: ") + dump_path);
        }
        binary.write(
            reinterpret_cast<const char *>(data.data()),
            static_cast<std::streamsize>(data.size()));
        if (!binary) {
            throw std::runtime_error("could not write raw-K direction dump");
        }
    }
    {
        std::ofstream meta(
            std::string(dump_path) + ".json", std::ios::trunc);
        if (!meta) {
            throw std::runtime_error("could not open raw-K dump metadata");
        }
        meta << "{\n"
             << "  \"schema\": \"qw3.raw_content_k_window.v1\",\n"
             << "  \"layout\": \"layer_token_kvhead_dim\",\n"
             << "  \"dtype\": \"" << (fp32 ? "float32" : "float16")
             << "\",\n"
             << "  \"token_begin\": " << begin << ",\n"
             << "  \"token_end\": " << end << ",\n"
             << "  \"n_layers\": " << kvmem_raw_layers_.size() << ",\n"
             << "  \"n_kv_heads\": " << cfg.n_kv_heads << ",\n"
             << "  \"head_dim\": " << cfg.head_dim << ",\n"
             << "  \"layers\": [";
        for (size_t slot = 0; slot < kvmem_raw_layers_.size(); ++slot) {
            if (slot != 0) meta << ", ";
            meta << kvmem_raw_layers_[slot];
        }
        meta << "]\n}\n";
        if (!meta) {
            throw std::runtime_error("could not write raw-K dump metadata");
        }
    }
    kvmem_raw_k_debug_dump_done_ = true;
    std::fprintf(
        stderr,
        "[kvmem-raw-k-dump] path=%s range=[%u,%u) layers=%zu "
        "kv_heads=%u head_dim=%u dtype=%s bytes=%zu\n",
        dump_path, begin, end, kvmem_raw_layers_.size(),
        cfg.n_kv_heads, cfg.head_dim, fp32 ? "float32" : "float16",
        data.size());
}
...
void QwenExecutor::kvmem_truncate_raw_k(uint32_t token_pos) {
    ...
    for (size_t i = first; i < chunk_count; ++i) {
        ...
        if (i < kvmem_raw_k_nvme_backed_.size()) {
            kvmem_raw_k_nvme_backed_[i] = 0;
        }
        if (i < kvmem_raw_mtp_k_nvme_backed_.size()) {
            kvmem_raw_mtp_k_nvme_backed_[i] = 0;
        }
        if (i < kvmem_raw_k_dirty_.size()) {
            kvmem_raw_k_dirty_[i] = 0;
        }
        if (i < kvmem_raw_mtp_k_dirty_.size()) {
            kvmem_raw_mtp_k_dirty_[i] = 0;
        }
    }
}
...
void QwenExecutor::kvmem_ensure_raw_capture_capacity(uint32_t rows) {
    ...
    const bool device_slots_ready = std::all_of(
        kvmem_raw_capture_dev_.begin(),
        kvmem_raw_capture_dev_.end(),
        [&](const auto &slot) {
            return slot.size() == kvmem_raw_layers_.size();
        });
    if (rows <= kvmem_raw_capture_rows_ &&
        device_slots_ready && kvmem_raw_capture_host_) {
        return;
    }
    for (auto &fence : kvmem_raw_capture_d2h_done_) {
        require_status(
            backend_.wait_kv_transfer_fence(fence));
    }
    const QwenConfig &cfg = model_.config();
    const uint64_t per_pos =
        static_cast<uint64_t>(cfg.n_kv_heads) * cfg.head_dim;
    kvmem_raw_capture_rows_ = std::max(rows, kvmem_raw_capture_rows_);
    for (uint32_t buffer = 0;
         buffer < kvmem_raw_capture_dev_.size(); ++buffer) {
        auto &capture = kvmem_raw_capture_dev_[buffer];
        capture.clear();
        capture.reserve(kvmem_raw_layers_.size());
        for (uint32_t slot = 0;
             slot < kvmem_raw_layers_.size(); ++slot) {
            const std::string label =
                "kvmem_raw_k_capture_b" +
                std::to_string(buffer) + "_l" +
                std::to_string(kvmem_raw_layers_[slot]);
            capture.push_back(kvmem_alloc_raw_k_tensor(
                static_cast<uint64_t>(
                    kvmem_raw_capture_rows_) * per_pos,
                label.c_str()));
        }
    }
    kvmem_raw_capture_active_slot_ = 0;
...
void QwenExecutor::kvmem_prepare_raw_capture_slot() {
    auto &fence = kvmem_raw_capture_d2h_done_[
        kvmem_raw_capture_active_slot_];
    if (!fence) return;
    // Queue the dependency on the execution stream instead of synchronizing
    // the host. CUDA keeps the event alive for the already-enqueued wait after
    // reset, so this capture buffer may be overwritten only after its previous
    // D2H has completed.
    require_status(
        backend_.execution_wait_for_kv_transfer(fence));
    fence.reset();
}
...
bool QwenExecutor::kvmem_try_direct_raw_k_d2h(
        uint32_t true_base, uint32_t first_row,
        uint32_t rows) {
    if (!kvmem_raw_k_nvme_enabled_ ||
        !kvmem_raw_k_block_major_ ||
        std::getenv("QW3_KVMEM_DUMP_RAW_K") ||
        rows != kvmem_raw_k_chunk_tokens_ ||
        true_base % kvmem_raw_k_chunk_tokens_ != 0 ||
        first_row + rows > kvmem_raw_capture_rows_ ||
        kvmem_raw_writeback_record_bytes_ == 0) {
        return false;
    }
    const uint32_t chunk =
        true_base / kvmem_raw_k_chunk_tokens_;
    if (chunk >= kvmem_raw_k_chunks_.size()) return false;
    kvmem_ensure_raw_k_chunks(
        true_base, rows, /*mtp=*/false);
    if (kvmem_raw_mtp_k_chunk_bytes_ > 0) {
        // Reserve the complete record's pageable cache footprint before the
        // main D2H takes ownership of a staging record. This keeps later MTP
        // capture from needing to reclaim a half-built slot under a tight CPU
        // budget.
        kvmem_ensure_raw_k_chunks(
            true_base, rows, /*mtp=*/true);
    }
    uint32_t slot_index = 0;
    uint64_t record_offset = 0;
    if (!kvmem_reserve_raw_k_writeback_record(
        ...
        for (uint32_t layer_slot = 0;
            ...
            require_status(backend_.copy_bytes_to_host_async(
                *kvmem_raw_capture_dev_[
                    kvmem_raw_capture_active_slot_][layer_slot],
                dst,
                src_offset, layer_block_bytes));
        }
    }
    require_status(backend_.record_kv_transfer_fence(
        kvmem_raw_capture_d2h_done_[
            kvmem_raw_capture_active_slot_]));
    kvmem_raw_capture_active_slot_ =
        (kvmem_raw_capture_active_slot_ + 1u) %
        static_cast<uint32_t>(
            kvmem_raw_capture_dev_.size());
    kvmem_raw_k_dirty_[chunk] = 1;
    kvmem_raw_writeback_main_ready_[chunk] = 1;
    std::fill(
        kvmem_raw_k_valid_tokens_.begin() + true_base,
        kvmem_raw_k_valid_tokens_.begin() + true_base + rows,
        static_cast<uint8_t>(1));
    kvmem_persist_completed_raw_k_chunks(
        true_base, rows, /*mtp=*/false);
    kvmem_maybe_dump_raw_k_window();
    return true;
}
...
void QwenExecutor::kvmem_flush_raw_k_capture(
    ...
    if (kvmem_try_direct_raw_k_d2h(
            true_base, first_row, rows)) {
        return;
    }
    ...
    for (uint32_t slot = 0; slot < kvmem_raw_layers_.size(); ++slot) {
        require_status(backend_.copy_bytes_to_host_async(
            *kvmem_raw_capture_dev_[
                kvmem_raw_capture_active_slot_][slot],
...
void QwenExecutor::kvmem_materialize_raw_k(
    ...
    for (uint32_t begin = 0; begin < refreshes.size(); begin += cap) {
        ...
        auto gather_block = [&](uint32_t j) {
            ...
            kvmem_read_raw_k_block(
                block.orig_pos_start, rm.n_tokens, dst,
                /*mtp=*/false);
        };
...
void QwenExecutor::kvmem_ensure_raw_mtp_capture_capacity(uint32_t rows) {
    ...
    const bool device_slots_ready = std::all_of(
        kvmem_raw_mtp_capture_dev_.begin(),
        kvmem_raw_mtp_capture_dev_.end(),
        [](const auto &slot) {
            return static_cast<bool>(slot);
        });
    if (rows <= kvmem_raw_mtp_capture_rows_ &&
        device_slots_ready && kvmem_raw_mtp_capture_host_) {
        return;
    }
    for (auto &fence : kvmem_raw_mtp_capture_d2h_done_) {
        require_status(
            backend_.wait_kv_transfer_fence(fence));
    }
    const QwenConfig &cfg = model_.config();
    ...
    for (uint32_t slot = 0;
         slot < kvmem_raw_mtp_capture_dev_.size(); ++slot) {
        kvmem_raw_mtp_capture_dev_[slot] =
            kvmem_alloc_raw_k_tensor(
                static_cast<uint64_t>(
                    kvmem_raw_mtp_capture_rows_) * per_pos,
                ("kvmem_raw_mtp_k_capture_" +
                 std::to_string(slot)).c_str());
    }
    kvmem_raw_mtp_capture_active_slot_ = 0;
...
bool QwenExecutor::kvmem_try_direct_raw_mtp_k_d2h(
        const DeviceTensor &raw_k,
        uint32_t logical_base, uint32_t rows) {
    if (!kvmem_raw_k_nvme_enabled_ ||
        rows != kvmem_raw_k_chunk_tokens_ ||
        logical_base % kvmem_raw_k_chunk_tokens_ != 0 ||
        kvmem_raw_mtp_k_chunk_bytes_ == 0) {
        return false;
    }
    const uint32_t chunk =
        logical_base / kvmem_raw_k_chunk_tokens_;
    if (chunk >= kvmem_raw_mtp_k_chunks_.size() ||
        chunk >= kvmem_raw_writeback_chunk_slot_.size() ||
        kvmem_raw_writeback_chunk_slot_[chunk] < 0 ||
        !kvmem_raw_writeback_main_ready_[chunk]) {
        return false;
    }
    kvmem_ensure_raw_k_chunks(
        logical_base, rows, /*mtp=*/true);
    ...
    kvmem_raw_mtp_capture_active_slot_ =
        (kvmem_raw_mtp_capture_active_slot_ + 1u) %
        static_cast<uint32_t>(
            kvmem_raw_mtp_capture_dev_.size());
    kvmem_raw_mtp_k_dirty_[chunk] = 1;
    kvmem_raw_writeback_mtp_ready_[chunk] = 1;
    std::fill(
...
void QwenExecutor::kvmem_capture_raw_mtp_k(
    ...
    if (kvmem_try_direct_raw_mtp_k_d2h(
            *kvmem_raw_mtp_capture_dev_[
                kvmem_raw_mtp_capture_active_slot_],
            logical_base, rows)) {
        return;
...
NativeExecutorReport QwenExecutor::forward_one_token(uint32_t token_id,
    ...
    for (uint32_t il = 0; il < weights_.n_layers(); ++il) {
        ...
        if (layer.recurrent) {
            ...
            if (kvmem_immutable_source_k_) {
                require_status(backend_.rmsnorm_per_head(
                    *q_, *layer.attn_q_norm, standard_n_heads,
                    2 * standard_head_dim, standard_head_dim, eps));
                require_status(backend_.rmsnorm_per_head(
                    *k_, *layer.attn_k_norm, standard_n_kv_heads,
                    standard_head_dim, standard_head_dim, eps));
                kvmem_capture_raw_k_decode(
                    il, *k_, position_ % block_store_->config().block_tokens);
                // Partial RoPE on the first rope_dim of each head's first
                // segment, baked at the active window position.
                ...
                // Fuse both per-head RMS norms with both partial RoPE
                // transforms. Q's second head_dim segment is the output gate
...
void QwenExecutor::kvmem_begin_query_replay(
        ...
        bool preserve_selected_context) {
    const bool trace =
        kvmem_perf_trace_flag() || std::getenv("QW3_KVMEM_TRACE");
    const uint64_t replay_prepare_start =
    ...
    uint32_t preserved_context_tokens = 0;
    uint32_t preserved_context_pages = 0;
    uint32_t dropped_suffix_blocks = 0;
    if (preserve_selected_context) {
        if (!kvmem_active_) {
            throw std::runtime_error(
                "KVMem query replay cannot preserve an inactive window");
        }
        std::vector<uint32_t> requested = context_block_ids;
        std::sort(requested.begin(), requested.end());
        if (std::adjacent_find(requested.begin(), requested.end()) !=
            requested.end()) {
            throw std::runtime_error(
                "KVMem query replay context selection contains duplicates");
        }
        std::vector<uint32_t> live_context;
        const uint32_t page_size = std::max<uint32_t>(
            1, kv_pages_.page_size);
        uint32_t expected_baked_pos = 0;
        for (const KvMemBlock &block : block_store_->blocks()) {
            if (!block.in_working_set) continue;
            if (block.orig_pos_start >= boundary.position) {
                ++dropped_suffix_blocks;
                continue;
            }
            live_context.push_back(block.block_id);
            if (block.baked_pos !=
                static_cast<int64_t>(expected_baked_pos)) {
                throw std::runtime_error(
                    "KVMem query replay selected context is not packed at "
                    "its current window prefix");
            }
            expected_baked_pos += block.n_tokens;
            const uint32_t first_page =
                block.orig_pos_start / page_size;
            const uint32_t last_page =
                (block.orig_pos_start + block.n_tokens - 1) /
                page_size;
            preserved_context_pages +=
                last_page - first_page + 1;
        }
        preserved_context_tokens = expected_baked_pos;
        if (requested != live_context) {
            throw std::runtime_error(
                "KVMem query replay preserve mode does not match the live "
                "semantic context selection");
        }
        if (bs_window_block_ids_.size() < live_context.size() ||
            !std::equal(live_context.begin(), live_context.end(),
                        bs_window_block_ids_.begin())) {
            throw std::runtime_error(
                "KVMem query replay window metadata does not begin with the "
                "selected context");
        }
        for (size_t i = live_context.size();
             i < bs_window_block_ids_.size(); ++i) {
            const uint32_t id = bs_window_block_ids_[i];
            if (id >= block_store_->block_count() ||
                block_store_->blocks()[id].orig_pos_start <
                    boundary.position) {
                throw std::runtime_error(
                    "KVMem query replay suffix is not a window tail");
            }
        }
        if (preserved_context_tokens > window_query_pos_ ||
            preserved_context_pages > window_pages_host_.size()) {
            throw std::runtime_error(
                "KVMem query replay context exceeds the assembled window");
        }
    } else {
        // Some experimental session-refresh paths deliberately replace the
        // live semantic context before replay. Keep the explicit-selection
        // behavior only for those callers; ordinary final-query replay uses
        // preserve_selected_context and never enters this path.
    ...
    if (preserve_selected_context) {
        // Semantic assembly orders selected blocks chronologically, hence every
        ...
        bs_window_block_ids_.resize(context_block_ids.size());
        bs_win_base_host_.resize(context_block_ids.size());
        bs_blk_tokens_host_.resize(context_block_ids.size());
        bs_window_blocks_ =
            static_cast<uint32_t>(context_block_ids.size());
    }
    ...
    if (boundary.mtp_prefix_h && mtp_prefix_h_) {
        require_status(backend_.copy_d2d(
    ...
    if (trace) {
        const uint64_t replay_prepare_end = kvmem_steady_ns();
        std::fprintf(stderr,
                     "[bs-query-replay] begin boundary=%u context_blocks=%zu "
                     "window_tokens=%u reset_recurrent=%d mode=%s "
                     "dropped_suffix_blocks=%u drain_ms=%.3f "
                     "rollback_ms=%.3f total_ms=%.3f\n",
                     boundary.position, context_block_ids.size(),
                     window_query_pos_, reset_recurrent_state ? 1 : 0,
                     preserve_selected_context
                         ? "preserve-selected" : "explicit-select",
                     dropped_suffix_blocks,
                     (replay_prepare_after_drain - replay_prepare_start) /
                         1.0e6,
                     (replay_prepare_end - replay_prepare_after_drain) /
                         1.0e6,
                     (replay_prepare_end - replay_prepare_start) / 1.0e6);
    }
}
...
void QwenExecutor::configure_kvmem(const KvMemStoreConfig &cfg) {
    ...
    // Pack controls the two-slot gather/H2D/scatter assembly pipeline.
    kvmem_raw_pipeline_enabled_ =
        kvmem_rope_table_enabled_ &&
        effective.optimize_pack;
    if (effective.optimize_stage_out &&
        effective.nvme_tier_bytes > 0 &&
        !kvmem_immutable_source_k_) {
        throw std::runtime_error(
            "KVMem stage-out optimization requires immutable raw K; "
    ...
    } else if (effective.raw_k_nvme) {
        throw std::runtime_error(
            "--kvmem-raw-k-nvme requires --kvmem-immutable-k");
    }
    if (kvmem_archive_ != nullptr) {
        // The archive stores position-free K, which only exists in immutable
        // mode, and only fp8 is small enough for the context lengths this is
        // built for (10M tokens is 305 GiB at fp8, 610 GiB at fp16).
        if (!kvmem_immutable_source_k_) {
            throw std::runtime_error(
                "a KVMem context archive requires --kvmem-immutable-k");
        }
        if (!kvmem_kv_dtype || std::strcmp(kvmem_kv_dtype, "fp8") != 0) {
            throw std::runtime_error(
                "a KVMem context archive requires fp8 KV (set "
                "QW3_KV_DTYPE=fp8)");
        }
        if (kvmem_archive_build_ && !effective.optimize_stage_out) {
            throw std::runtime_error(
                "building a KVMem context archive requires "
                "--kvmem-opt-stage-out on so V reaches the arena before a "
                "block is evicted");
        }
        // An attached archive is immutable, but the session still writes: the
        // residual re-prefill, the question, and the decode all make new KV
        // that may be evicted. Those writes go to a scratch overlay beside the
        // arena, keeping the archive reusable by other processes.
        if (!kvmem_archive_build_) {
            kvmem_archive_overlay_dir_ =
                effective.nvme_tier_dir.empty()
                    ? std::string("/tmp") : effective.nvme_tier_dir;
        }
        // Both arenas move into the archive directory; their capacity is what
        // the context needs, not a cache budget.
        effective.raw_k_nvme = true;
        effective.nvme_tier_dir = kvmem_archive_->dir();
    }
    if (!kvmem_immutable_source_k_) {
        if (effective.optimize_stage_out) {
            throw std::runtime_error(
                "KVMem stage-out optimization requires immutable raw K; "
                "remove --no-kvmem-immutable-k or pass "
                "--kvmem-opt-stage-out off");
        }
        if (effective.optimize_pack) {
            throw std::runtime_error(
                "KVMem pack optimization requires immutable raw K; "
                "remove --no-kvmem-immutable-k or pass "
                "--kvmem-opt-pack off");
        }
        if (!effective.optimize_stage_in) {
            throw std::runtime_error(
                "KVMem stage-in-off requires immutable raw K "
    ...
    // raw authority reproduced the stable rebuild path byte-for-byte.
    // Incremental MTP assembly is not a performance-ablation dimension because
    ...
    uint64_t nvme_spill_bytes = effective.nvme_tier_bytes;
    uint64_t raw_authority_bytes = 0;
    ...
    if (kvmem_immutable_source_k_) {
        ...
        if (kvmem_archive_ != nullptr) {
            const uint64_t raw_record_bytes =
                raw_chunk_count == 0
                    ? 0 : raw_authority_bytes / raw_chunk_count;
            if (raw_record_bytes == 0 ||
                effective.cpu_tier_bytes < raw_record_bytes) {
                throw std::runtime_error(
                    "a KVMem context archive needs enough --kvmem-cpu-gb for "
                    "at least one raw-K staging chunk (" +
                    std::to_string(raw_record_bytes) + " bytes)");
            }
            // The V arena must hold every context block, because a
            // direct-mapped archive never evicts.
            const uint64_t ctx_blocks =
                (static_cast<uint64_t>(kv_ctx_size_) +
                 effective.block_tokens - 1) / effective.block_tokens;
            nvme_spill_bytes = ctx_blocks * effective.estimated_block_bytes;
            capacity_cpu_tier_bytes = 0;
        } else if (effective.raw_k_nvme) {
            if (effective.nvme_tier_dir.empty() ||
                effective.nvme_tier_bytes == 0) {
                throw std::runtime_error(
                    "--kvmem-raw-k-nvme requires --kvmem-nvme-dir and "
                    "a non-zero --kvmem-nvme-gb");
            }
            const uint64_t raw_record_bytes =
                raw_chunk_count == 0
                    ? 0 : raw_authority_bytes / raw_chunk_count;
            if (raw_record_bytes == 0 ||
                effective.cpu_tier_bytes < raw_record_bytes) {
                throw std::runtime_error(
                    "KVMem raw-K NVMe needs enough --kvmem-cpu-gb for at "
                    "least one raw-K staging/cache chunk (" +
                    std::to_string(raw_record_bytes) + " bytes)");
            }
            if (effective.nvme_tier_bytes < raw_authority_bytes) {
                throw std::runtime_error(
                    "KVMem raw-K NVMe authority requires " +
                    std::to_string(raw_authority_bytes) +
                    " bytes, exceeding the total --kvmem-nvme-gb budget (" +
                    std::to_string(effective.nvme_tier_bytes) +
                    " bytes). Raise --kvmem-nvme-gb or lower --ctx.");
            }
            // Raw-K owns a fixed-capacity arena at the front of the logical
            // NVMe budget. The remainder is available for ordinary V records.
            nvme_spill_bytes =
                effective.nvme_tier_bytes - raw_authority_bytes;
            // Raw chunks may use the whole host budget as a cache, so none of
            // it is a guaranteed V capacity at admission time. Stage-out's
            // authoritative V arena is validated independently below.
            capacity_cpu_tier_bytes = 0;
        } else {
            if (effective.cpu_tier_bytes < raw_authority_bytes) {
                throw std::runtime_error(
                    ...
                    " bytes). Raise --kvmem-cpu-gb, lower --ctx, enable "
                    "--kvmem-raw-k-nvme, or use fp8 KV.");
            }
            // Runtime raw-K and CPU V blocks share the full host budget
            // dynamically. Capacity validation reserves the worst-case raw-K
            // authority when SSD raw-K backing is disabled.
    ...
    // Index placement is part of the allocation topology. Reconfiguration may
    // switch between a resident full-device index and a pageable CPU authority
    // with two bounded pinned/device staging slots, so discard the old topology
    // rather than retaining its VRAM/host high-water mark.
    g_kbar_multi_.reset();
    g_kbar_multi_capacity_ = 0;
    g_kbar_multi_host_.reset();
    ...
    g_kbar_stream_tile_blocks_ = 0;
    g_kbar_stream_slot_bytes_ = 0;
    g_kbar_capture_next_slot_ = 0;
    g_kbar_capture_active_slot_ = -1;
    g_kbar_stream_global_max_.reset();
    g_kbar_stream_global_sum_.reset();
    g_kbar_stream_distribution_capacity_ = 0;
    g_kbar_stream_group_dist_.reset();
    g_kbar_stream_group_dist_capacity_ = 0;
    kvmem_active_ = false;
    window_pages_host_.clear();
    window_page_count_ = 0;
    mtp_window_pages_host_.clear();
    ...
    kvmem_raw_mtp_k_dirty_.clear();
    kvmem_raw_k_write_pending_.clear();
    kvmem_raw_k_nvme_read_bytes_ = 0;
    kvmem_raw_k_nvme_write_bytes_ = 0;
    kvmem_raw_k_nvme_write_syscalls_ = 0;
    kvmem_raw_k_nvme_write_ns_ = 0;
    for (auto &slot : kvmem_raw_writeback_slots_) {
        slot = RawWritebackSlot{};
    }
    kvmem_raw_writeback_fill_slot_ = 0;
    kvmem_raw_writeback_chunks_per_slot_ = 0;
    kvmem_raw_writeback_record_bytes_ = 0;
    kvmem_raw_writeback_chunk_slot_.clear();
    kvmem_raw_writeback_chunk_offset_.clear();
    kvmem_raw_writeback_main_ready_.clear();
    kvmem_raw_writeback_mtp_ready_.clear();
    kvmem_raw_k_valid_tokens_.clear();
    kvmem_raw_k_debug_dump_done_ = false;
    kvmem_raw_layers_.clear();
    kvmem_raw_layer_slot_.clear();
    for (auto &slot : kvmem_raw_capture_dev_) {
        slot.clear();
    }
    for (auto &fence : kvmem_raw_capture_d2h_done_) {
        fence.reset();
    ...
    for (auto &slot : kvmem_raw_mtp_capture_dev_) {
        slot.reset();
    }
    for (auto &fence : kvmem_raw_mtp_capture_d2h_done_) {
        fence.reset();
    ...
    if (kvmem_immutable_source_k_) {
        ...
        kvmem_raw_k_dirty_.assign(
            static_cast<size_t>(raw_chunk_count), 0);
        kvmem_raw_k_write_pending_.assign(
            static_cast<size_t>(raw_chunk_count), 0);
        if (kvmem_mtp_local_positions_) {
            kvmem_raw_mtp_k_chunks_.resize(
                static_cast<size_t>(raw_chunk_count));
            kvmem_raw_mtp_k_valid_tokens_.assign(kv_ctx_size_, 0);
            kvmem_raw_mtp_k_nvme_backed_.assign(
                static_cast<size_t>(raw_chunk_count), 0);
            kvmem_raw_mtp_k_dirty_.assign(
                static_cast<size_t>(raw_chunk_count), 0);
        }
        if (effective.raw_k_nvme) {
            ...
            if (kvmem_archive_ != nullptr) {
                // Slot == chunk id, so the archive needs no persisted slot
                // table and a reader recovers the mapping arithmetically.
                raw_cfg.file_name = kvmem_archive_files::kRawK;
                raw_cfg.durable = true;
                raw_cfg.direct_mapped = true;
                raw_cfg.read_only = !kvmem_archive_build_;
                raw_cfg.preallocate = kvmem_archive_build_;
                raw_cfg.overlay_dir = kvmem_archive_overlay_dir_;
                raw_cfg.overlay_file_name = "qw3_kvmem_rawk_overlay.bin";
                raw_cfg.direct_read = env_flag_enabled(
                    "QW3_KVMEM_DIRECT_READ", true);
            }
            ...
            kvmem_raw_writeback_chunks_per_slot_ =
                effective.optimize_pack
                    ? static_cast<uint32_t>(std::max<uint64_t>(
                          1, kvmem_io_slab_bytes() /
                                 kvmem_raw_writeback_record_bytes_))
                    : 1u;
            const uint64_t writeback_slot_bytes =
                static_cast<uint64_t>(
                    kvmem_raw_writeback_chunks_per_slot_) *
                kvmem_raw_writeback_record_bytes_;
            for (uint32_t i = 0;
                ...
                RawWritebackSlot &slot =
                    kvmem_raw_writeback_slots_[i];
                slot.host = backend_.host_buffer(
                    writeback_slot_bytes,
                    ("kvmem_raw_k_writeback_" +
                     std::to_string(i)).c_str());
                slot.capacity_bytes = writeback_slot_bytes;
            }
            kvmem_raw_writeback_chunk_slot_.assign(
                static_cast<size_t>(raw_chunk_count),
                static_cast<int8_t>(-1));
            kvmem_raw_writeback_chunk_offset_.assign(
                static_cast<size_t>(raw_chunk_count), 0);
            kvmem_raw_writeback_main_ready_.assign(
                static_cast<size_t>(raw_chunk_count), 0);
            kvmem_raw_writeback_mtp_ready_.assign(
                static_cast<size_t>(raw_chunk_count), 0);
            std::fprintf(
                stderr,
                ...
                static_cast<unsigned long long>(
                    raw_authority_bytes),
                static_cast<unsigned long long>(nvme_spill_bytes),
                static_cast<unsigned long long>(
                    effective.cpu_tier_bytes),
                kvmem_raw_writeback_chunks_per_slot_,
                static_cast<unsigned long long>(
                    writeback_slot_bytes),
                kvmem_raw_k_block_major_ ? 1 : 0);
        }
        if (!effective.optimize_pack) {
    ...
    const bool heat_aware_cpu_cache = effective.optimize_stage_in;
    ...
    // SSD-backed stage-out already has its own inclusive authority and does
    // not use this flag.
    if (effective.optimize_stage_out &&
        kvmem_cpu_tier_ && nvme_spill_bytes == 0 &&
        effective.estimated_block_bytes > 0) {
        const uint64_t ctx_blocks =
            (static_cast<uint64_t>(kv_ctx_size_) +
    ...
    if (nvme_spill_bytes >= effective.estimated_block_bytes &&
        effective.estimated_block_bytes > 0) {
        if (effective.nvme_tier_dir.empty()) {
            throw std::runtime_error(
        ...
        ncfg.total_bytes = nvme_spill_bytes;
        ncfg.slot_bytes = effective.estimated_block_bytes;
        ncfg.drop_page_cache = env_flag_enabled(
            "QW3_KVMEM_DROP_PAGE_CACHE", true);
        if (kvmem_archive_ != nullptr) {
            // Slot == block id, matching the raw-K arena's chunk mapping.
            ncfg.file_name = kvmem_archive_files::kV;
            ncfg.durable = true;
            ncfg.direct_mapped = true;
            ncfg.read_only = !kvmem_archive_build_;
            ncfg.preallocate = kvmem_archive_build_;
            ncfg.overlay_dir = kvmem_archive_overlay_dir_;
            ncfg.overlay_file_name = "qw3_kvmem_v_overlay.bin";
            ncfg.direct_read = env_flag_enabled(
                "QW3_KVMEM_DIRECT_READ", true);
        }
        kvmem_nvme_tier_ = std::make_unique<NvmeKvTier>(std::move(ncfg));
        std::fprintf(
            stderr,
            "[kvmem-io] page_cache_policy=%s reason=%s direct_read=%d\n",
            kvmem_nvme_tier_->drops_page_cache()
                ? "writeback-and-drop-after-io" : "kernel-default",
            kvmem_nvme_tier_->drops_page_cache()
                ? "bound-host-memory" : "explicit-env-override",
            kvmem_nvme_tier_->direct_reads() ? 1 : 0);
        if (effective.optimize_pack) {
    ...
    const bool proactive_d2h = effective.optimize_stage_out;
    const bool packed_d2h = effective.optimize_pack;
    const bool async_ssd_write = proactive_d2h && has_ssd;
    const bool pipelined_h2d = effective.optimize_pack;
    ...
    if (tiering_active) {
        if (effective.optimize_stage_out &&
            ...
            throw std::runtime_error(
                "KVMem stage-out optimization was requested but cannot be "
                "enabled: " + reason +
                "; fix the tier configuration or pass "
                "--kvmem-opt-stage-out off");
        }
        if (effective.optimize_pack &&
            (!kvmem_raw_pipeline_enabled_ ||
             std::strcmp(backend_.name(), "cuda-device") != 0)) {
            throw std::runtime_error(
                "KVMem pack optimization was requested but the "
                "backend does not provide the required packed "
                "gather/H2D/GPU-scatter pipeline; pass "
                "--kvmem-opt-pack off");
        }
    }
    ...
    log_opt_status(
        "stage-out", effective.optimize_stage_out,
        kvmem_prefill_writeback_enabled_
            ? (has_ssd ? "target=ssd" : "target=cpu")
            : "target=none");
    log_opt_status(
        "stage-in", effective.optimize_stage_in,
        effective.optimize_stage_in
            ? (kvmem_cpu_tier_
                   ? (heat_aware_cpu_cache
    ...
    std::fprintf(
        stderr,
        "[kvmem-opt] mode=%s stage_out=%d stage_in=%d pack=%d "
        "backing_mode=%s cpu_cache_policy=%s "
        "proactive_d2h=%d packed_d2h=%d inclusive_cpu=%d "
        "async_ssd_write=%d "
        ...
        effective.optimize_stage_out ? 1 : 0,
        effective.optimize_stage_in ? 1 : 0,
        effective.optimize_pack ? 1 : 0,
        effective.optimize_stage_out ? "inclusive" : "exclusive",
        heat_aware_cpu_cache
    ...
    if (kvmem_prefill_writeback_enabled_) {
        const uint64_t warm0 = kvmem_steady_ns();
        const size_t slab_bytes = effective.optimize_pack
            ? kvmem_io_slab_bytes()
            : static_cast<size_t>(std::max<uint64_t>(
                  1, effective.estimated_block_bytes));
    ...
    if (packed_d2h) {
        ...
        for (size_t i = 0; i < kStageOutSlabs; ++i) {
            kvmem_free_stageout_slabs_.push_back(
                backend_.host_buffer(
                    slab_bytes, "kvmem_stageout_packed_slab"));
        }
        std::fprintf(
            stderr,
            "[kvmem-io] packed_stageout_slab_pool slabs=%zu slab_mib=%zu "
    ...
    if (kvmem_gpu_page_pool_ && effective.estimated_block_bytes > 0) {
        ...
        if (available_blocks < required_blocks) {
            ...
            if (inclusive) {
                throw std::runtime_error(
                    "kvmem: stage-out inclusive backing is too small. A "
...
void QwenExecutor::kvmem_build_index_from_raw_k(uint32_t blocks) {
    if (!block_store_ || blocks == 0) return;
    if (!kvmem_qc_capture_active_) {
        throw std::runtime_error(
            "rebuilding the KVMem index from raw K requires an active capture "
            "span; call kvmem_set_query_span first");
    }
    const QwenConfig &cfg = model_.config();
    const uint32_t bt =
        std::max<uint32_t>(1, block_store_->config().block_tokens);
    ...
    // The index is policy, not layout: rebuilding it here is what lets an
    // attach change retrieval method, sub-block count, or adaptive thresholds
    // without reprocessing the context. Batch enough blocks per pass that the
    // SSD sees large sequential reads.
    uint32_t rows_per_batch = kvmem_raw_k_chunk_tokens_;
    rows_per_batch = std::max(rows_per_batch, bt);
    rows_per_batch = (rows_per_batch / bt) * bt;
    const uint64_t total_tokens = static_cast<uint64_t>(blocks) * bt;

    const uint64_t raw_elems =
        static_cast<uint64_t>(rows_per_batch) * row_elems;
    if (!kvmem_index_rebuild_raw_dev_) {
        kvmem_index_rebuild_raw_dev_ =
            kvmem_alloc_raw_k_tensor(raw_elems, "kvmem_index_rebuild_raw");
        kvmem_index_rebuild_f32_dev_ =
            backend_.tensor_f32(raw_elems, "kvmem_index_rebuild_f32");
        kvmem_index_rebuild_host_ = backend_.host_buffer(
            static_cast<uint64_t>(rows_per_batch) * row_bytes,
            "kvmem_index_rebuild_host");
        kvmem_index_rebuild_chunk_host_ = backend_.host_buffer(
            kvmem_raw_k_chunk_bytes_,
            "kvmem_index_rebuild_chunk_host");
    }

    const uint64_t t0 = kvmem_steady_ns();
    kvmem_index_rebuild_source_ = kvmem_index_rebuild_f32_dev_.get();
    struct SourceScope {
        const DeviceTensor **slot;
        ~SourceScope() { *slot = nullptr; }
    } scope{&kvmem_index_rebuild_source_};

    uint8_t *host = static_cast<uint8_t *>(kvmem_index_rebuild_host_->data);
    uint8_t *chunk_host = static_cast<uint8_t *>(
        kvmem_index_rebuild_chunk_host_->data);
    uint64_t read_ns = 0;
    uint64_t gather_ns = 0;
    for (uint64_t base = 0; base < total_tokens; base += rows_per_batch) {
        const uint32_t rows = static_cast<uint32_t>(
            std::min<uint64_t>(rows_per_batch, total_tokens - base));
        const uint32_t chunk = static_cast<uint32_t>(
            base / kvmem_raw_k_chunk_tokens_);
        if (base % kvmem_raw_k_chunk_tokens_ != 0 ||
            rows_per_batch != kvmem_raw_k_chunk_tokens_ ||
            chunk >= kvmem_raw_k_chunks_.size()) {
            throw std::runtime_error(
                "archive index rebuild lost raw-K chunk alignment");
        }

        // The archive is block-major: reading one layer at a time produces
        // 128-KiB reads separated by every other layer in the record. Read the
        // complete main-K segment once (32 MiB in the reference layout), then
        // transpose only the requested layer into the bounded H2D buffer.
        const uint64_t read_begin = kvmem_steady_ns();
        if (kvmem_raw_k_chunks_[chunk]) {
            std::memcpy(chunk_host, kvmem_raw_k_chunks_[chunk].get(),
                        static_cast<size_t>(kvmem_raw_k_chunk_bytes_));
        } else {
            if (!kvmem_raw_k_nvme_enabled_ ||
                chunk >= kvmem_raw_k_nvme_backed_.size() ||
                ...
                !kvmem_raw_k_nvme_tier_) {
                throw std::runtime_error(
                    "archive index rebuild raw-K chunk has no authority");
            }
            const int32_t nvme_slot =
                kvmem_raw_k_nvme_tier_->block_slot(chunk);
            if (nvme_slot < 0) {
                throw std::runtime_error(
                    "archive index rebuild raw-K chunk lost its NVMe slot");
            }
            kvmem_raw_k_nvme_tier_->read_slot_range(
                nvme_slot, /*slot_byte_offset=*/0, chunk_host,
                kvmem_raw_k_chunk_bytes_);
        }
        read_ns += kvmem_steady_ns() - read_begin;

        const uint32_t physical_blocks =
            (rows + bt - 1) / bt;
        for (uint32_t slot = 0; slot < layers; ++slot) {
            const uint64_t gather_begin = kvmem_steady_ns();
            for (uint32_t block = 0; block < physical_blocks; ++block) {
                const uint32_t block_rows = std::min<uint32_t>(
                    bt, rows - block * bt);
                const uint64_t src_row =
                    (static_cast<uint64_t>(block) * layers + slot) * bt;
                const uint64_t dst_row = static_cast<uint64_t>(block) * bt;
                std::memcpy(host + dst_row * row_bytes,
                            chunk_host + src_row * row_bytes,
                            static_cast<size_t>(block_rows * row_bytes));
            }
            gather_ns += kvmem_steady_ns() - gather_begin;
            require_status(backend_.copy_bytes_from_host(
                *kvmem_index_rebuild_raw_dev_, /*byte_offset=*/0, host,
                static_cast<uint64_t>(rows) * row_bytes));
            require_status(backend_.widen_raw_k_to_f32_device(
                *kvmem_index_rebuild_raw_dev_, *kvmem_index_rebuild_f32_dev_,
                rows, static_cast<uint32_t>(row_elems)));
            kvmem_capture_kbar_multi(slot, rows, static_cast<uint32_t>(base),
                                     /*rope_base_pos=*/0,
                                     static_cast<uint32_t>(row_elems));
        }
    }
    require_status(backend_.synchronize());
    // Archive reconstruction is outside per-query TTFT. Move the completed
    // CPU Adaptive authority to pinned memory now so every later query can
    // stream it directly to the GPU without a 33+ GiB pageable bounce copy.
    kvmem_migrate_adaptive_index_to_pinned();
    std::fprintf(stderr,
                 "[kvmem-archive] index_rebuild blocks=%u tokens=%llu "
                 "layers=%u rows_per_batch=%u elapsed_ms=%.1f "
                 "read_ms=%.1f gather_ms=%.1f other_ms=%.1f\n",
                 blocks, static_cast<unsigned long long>(total_tokens), layers,
                 rows_per_batch, (kvmem_steady_ns() - t0) / 1.0e6,
                 read_ns / 1.0e6, gather_ns / 1.0e6,
...
uint64_t QwenExecutor::kvmem_attach_archive(uint64_t tokens) {
    ...
    // mode here would make an interrupted archive impossible to resume.
    if (!kvmem_archive_) {
        throw std::runtime_error("no KVMem context archive is attached");
    }
    if (!kvmem_enabled_ || !block_store_) {
        throw std::runtime_error(
            "attaching a KVMem context archive requires --kvmem");
    }
    const KvMemArchiveManifest &m = kvmem_archive_->manifest();
    if (tokens == 0 || tokens > m.total_tokens) tokens = m.total_tokens;
    if (tokens > kv_ctx_size_) {
        throw std::runtime_error(
            "KVMem archive holds " + std::to_string(tokens) +
            " tokens, which exceeds --ctx (" + std::to_string(kv_ctx_size_) +
            ")");
    }

    // DeltaNet recurrence is sequential, so an arbitrary prefix length can only
    // be reached from a ladder point at or below it; the caller re-prefills the
    // residual from the archived token stream.
    uint64_t restore_pos = kvmem_archive_->ladder_at_or_below(tokens);
    if (restore_pos == UINT64_MAX) restore_pos = 0;

    const uint32_t bt = block_store_->config().block_tokens;
    const uint32_t blocks =
        static_cast<uint32_t>((restore_pos + bt - 1) / bt);
    const uint32_t chunks = static_cast<uint32_t>(
        (restore_pos + kvmem_raw_k_chunk_tokens_ - 1) /
        kvmem_raw_k_chunk_tokens_);
    for (uint32_t c = 0; c < chunks; ++c) {
        if (!kvmem_archive_->chunk_valid(c)) {
            throw std::runtime_error(
                "KVMem archive raw-K chunk " + std::to_string(c) +
                " is missing; the archive is incomplete");
        }
    }
    for (uint32_t b = 0; b < blocks; ++b) {
        if (!kvmem_archive_->block_valid(b)) {
            throw std::runtime_error(
                "KVMem archive V block " + std::to_string(b) +
                " is missing; the archive is incomplete");
        }
    }

    // Every block is cold on SSD at its identity slot. This is exactly the
    // state a pressure-evicted prefill ends in, so nothing downstream has to
    // learn a new residency shape.
    block_store_->register_append(static_cast<uint32_t>(restore_pos));
    kvmem_registered_pos_ = static_cast<uint32_t>(restore_pos);
    if (kvmem_nvme_tier_) kvmem_nvme_tier_->mark_present_range(0, blocks);
    for (uint32_t b = 0; b < blocks; ++b) {
        block_store_->set_block_tier(b, KvTier::SSD, -1,
                                     static_cast<int32_t>(b));
        block_store_->set_block_ssd_backing(b, static_cast<int32_t>(b),
                                            /*clean=*/true);
    }

    // Raw K is the authority for K, and it stays on SSD until a block is
    // actually selected into a window.
    if (kvmem_raw_k_nvme_tier_) {
        kvmem_raw_k_nvme_tier_->mark_present_range(0, chunks);
    }
    if (kvmem_raw_k_valid_tokens_.size() < restore_pos) {
        kvmem_raw_k_valid_tokens_.resize(static_cast<size_t>(restore_pos), 0);
    }
    std::fill(kvmem_raw_k_valid_tokens_.begin(),
              kvmem_raw_k_valid_tokens_.begin() +
                  static_cast<ptrdiff_t>(restore_pos),
              static_cast<uint8_t>(1));
    for (uint32_t c = 0; c < chunks; ++c) {
        if (c < kvmem_raw_k_nvme_backed_.size()) {
            kvmem_raw_k_nvme_backed_[c] = 1;
            kvmem_raw_k_dirty_[c] = 0;
            kvmem_raw_k_write_pending_[c] = 0;
        }
    }
    // The MTP layer rides the same arena in the second segment of each raw-K
    // record, so it becomes durable and attachable at the same granularity.
    if (kvmem_mtp_tiered_) {
        if (kvmem_raw_mtp_k_valid_tokens_.size() < restore_pos) {
            kvmem_raw_mtp_k_valid_tokens_.resize(
                static_cast<size_t>(restore_pos), 0);
        }
        std::fill(kvmem_raw_mtp_k_valid_tokens_.begin(),
                  kvmem_raw_mtp_k_valid_tokens_.begin() +
                      static_cast<ptrdiff_t>(restore_pos),
                  static_cast<uint8_t>(1));
        for (uint32_t c = 0; c < chunks; ++c) {
            if (c < kvmem_raw_mtp_k_nvme_backed_.size()) {
                kvmem_raw_mtp_k_nvme_backed_[c] = 1;
                kvmem_raw_mtp_k_dirty_[c] = 0;
            }
        }
    }

    // The KV page table describes a fully non-resident history: pages exist
    // logically so positions resolve, but none is backed until a stage-in.
    ensure_scratch();
    // Pre-sizing the page vectors to the restored length (rather than letting
    // ensure_pages grow them) is what keeps the attach from allocating a
    // physical page per history token out of the bounded pool.
    auto install_absent_pages = [&](KvPageTable &table, const char *label) {
        const uint32_t page_size = table.page_size;
        if (page_size == 0) return;
        const uint32_t logical_pages = static_cast<uint32_t>(
            (restore_pos + page_size - 1) / page_size);
        table.reset();
        if (logical_pages == 0) return;
        table.pages.assign(logical_pages, -1);
        table.owned.assign(logical_pages, false);
        table.device_synced = 0;
        if (!table.device_pages) {
            table.device_pages = backend_.tensor_i32(
                std::max<uint32_t>(table.max_pages, 1), label);
        }
        require_status(backend_.copy_i32_from_host(
            *table.device_pages, /*dst_offset=*/0, table.pages.data(),
            logical_pages));
        table.device_synced = logical_pages;
    };
    install_absent_pages(kv_pages_, "kv_page_indices");
    if (kvmem_mtp_tiered_) {
        install_absent_pages(mtp_kv_pages_, "mtp_kv_page_indices");
    }

    if (restore_pos > 0) kvmem_archive_restore_ladder(restore_pos);
    position_ = static_cast<uint32_t>(restore_pos);
    kvmem_archive_attached_tokens_ = restore_pos;
    std::fprintf(stderr,
                 "[kvmem-archive] attach dir=%s requested_tokens=%llu "
                 "restored_tokens=%llu residual_tokens=%llu blocks=%u "
                 "chunks=%u archive_tokens=%llu\n",
                 kvmem_archive_->dir().c_str(),
                 static_cast<unsigned long long>(tokens),
                 static_cast<unsigned long long>(restore_pos),
                 static_cast<unsigned long long>(tokens - restore_pos), blocks,
                 chunks, static_cast<unsigned long long>(m.total_tokens));
    return restore_pos;
}
...
void QwenExecutor::kvmem_start_prefetch(const KvMemPlan &plan) {
    ...
    try {
        ...
        if ((kvmem_prefetch_.bulk_cpu &&
            ...
            // page-table H2D uploads from ordinary packed stage-in.
...
void QwenExecutor::kvmem_finish_prefetch() {
    ...
    uint32_t staged = 0;
    const bool stage_mtp_pages = kvmem_prefetch_.stage_mtp_pages;
    struct CpuCacheAdmissionSource {
        uint32_t block_id = 0;
        const uint8_t *src = nullptr;
        uint64_t bytes = 0;
    };
    // A sealed archive is an immutable SSD authority.  Stage-in used to copy
    // its records only SSD -> pinned slab -> GPU and then immediately recycle
    // the slab.  Consequently optimize_stage_in's heat-aware CPU cache never
    // admitted an archive block: every question reread the same hot records
    // from SSD even when tens of GiB of CPU capacity were idle.
    //
    // Admit directly from the completed read slab.  Metadata placement is
    // serialized here, while disjoint host copies use the bounded stage-in
    // worker pool.  The caller invokes this after queuing H2D and before its
    // fence wait, so CPU admission and PCIe transfer of the same read-only
    // bytes overlap.  SSD remains the clean authority; CPU is only a cache.
    auto admit_ssd_sources_to_cpu = [&](const std::vector<
                                            CpuCacheAdmissionSource> &sources) {
        if (sources.empty() || !kvmem_cpu_tier_ || !kvmem_nvme_tier_ ||
            !block_store_->config().optimize_stage_in) {
            return;
        }
        struct CopyTarget {
            uint32_t block_id = 0;
        ...
        const uint64_t admit0 =
            perf_trace ? kvmem_steady_ns() : 0;
        std::vector<CopyTarget> targets;
        targets.reserve(sources.size());
        std::unordered_map<int32_t, size_t> slot_target;
        slot_target.reserve(sources.size());
        const auto &blocks = block_store_->blocks();
        for (const CpuCacheAdmissionSource &source : sources) {
            if (!source.src || source.bytes == 0 ||
                source.block_id >= blocks.size()) {
                continue;
            }
            const KvMemBlock &source_block = blocks[source.block_id];
            const bool source_backed =
                source_block.ssd_clean && source_block.nvme_slot >= 0 &&
                kvmem_nvme_tier_->block_slot(source.block_id) ==
                    source_block.nvme_slot;
            if (!source_backed) {
                // Admission is allowed to be dropped, but it must never turn
                // a transient/overlay read into the only CPU authority.
                continue;
            }

            PinnedSlotPlacement placement;
            if (kvmem_sparse_cpu_tier_) {
                if (kvmem_cpu_budget_has(source.bytes)) {
                    placement =
                        kvmem_cpu_tier_->place_block(source.block_id);
                    if (placement.slot >= 0 &&
                        !kvmem_reserve_cpu_slot(placement.slot)) {
                        kvmem_cpu_tier_->release_block(source.block_id);
                        placement = kvmem_cpu_tier_->place_block_replacing(
                            source.block_id);
                    }
                } else {
                    placement = kvmem_cpu_tier_->place_block_replacing(
                ...
                placement =
                    kvmem_cpu_tier_->place_block_evicting(source.block_id);
            }
            ...
            if (placement.evicted_block >= 0) {
                ...
                if (victim >= blocks.size()) {
                    throw std::runtime_error(
                        "KVMem SSD stage-in CPU admission evicted an "
                        "unknown block");
                }
                ...
                if (!victim_backed) {
                    throw std::runtime_error(
                        "KVMem SSD stage-in CPU admission would evict "
            ...
            if (!dst) {
                throw std::runtime_error(
                    "KVMem SSD stage-in CPU admission lost its slot");
            }
            block_store_->set_block_cpu_copy(
                source.block_id, placement.slot);
            CopyTarget target{source.block_id, placement.slot, dst,
                              source.src, source.bytes};
            const auto existing = slot_target.find(placement.slot);
            if (existing == slot_target.end()) {
                slot_target.emplace(placement.slot, targets.size());
                targets.push_back(target);
            } else {
                // A full heat-aware cache can reuse one slot more than once
                // while walking a read batch.  Only the final owner may write
                // it; otherwise parallel memcpy workers would race.
                targets[existing->second] = target;
            }
        }

        auto copy_range = [&](size_t first, size_t last) {
            for (size_t i = first; i < last; ++i) {
                const CopyTarget &target = targets[i];
                std::memcpy(target.dst, target.src, target.bytes);
            }
        };
        const size_t worker_count = std::min(
            kvmem_overlap_stagein_threads(), targets.size());
        if (worker_count <= 1) {
            copy_range(0, targets.size());
        } else if (kvmem_stagein_worker_pool_) {
            kvmem_stagein_worker_pool_->run(
                worker_count, worker_count, [&](size_t worker) {
                    const size_t first =
                        targets.size() * worker / worker_count;
                    const size_t last =
                        targets.size() * (worker + 1) / worker_count;
            ...
            for (size_t worker = 0; worker < worker_count; ++worker) {
                ...
                const size_t last =
                    targets.size() * (worker + 1) / worker_count;
                workers.emplace_back(copy_range, first, last);
            }
            for (std::thread &worker : workers) worker.join();
        }
        if (perf_trace) {
            kvmem_prefetch_.perf.cpu_cache_admit_ns +=
                kvmem_steady_ns() - admit0;
            kvmem_prefetch_.perf.cpu_cache_admit_blocks +=
                static_cast<uint32_t>(targets.size());
            for (const CopyTarget &target : targets) {
                kvmem_prefetch_.perf.cpu_cache_admit_bytes +=
                    target.bytes;
            }
        }
    };
    try {
        ...
        if (kvmem_prefetch_.bulk_nvme) {
            while (!kvmem_prefetch_.nvme_batches.empty()) {
                ...
                std::vector<CpuCacheAdmissionSource> admission_sources;
                admission_sources.reserve(read_end - read_begin);
                const auto *slab =
                    static_cast<const uint8_t *>(host->data);
                for (size_t i = read_begin; i < read_end; ++i) {
                    const KvMemPrefetchNvmeRead &read =
                        kvmem_prefetch_.nvme_reads[i];
                    admission_sources.push_back(
                        CpuCacheAdmissionSource{
                            read.block_id, slab + read.batch_offset,
                            read.bytes});
                }
                admit_ssd_sources_to_cpu(admission_sources);
        ...
        if (!kvmem_prefetch_.bulk_nvme &&
            !kvmem_prefetch_.nvme_reads.empty()) {
            std::vector<CpuCacheAdmissionSource> admission_sources;
            admission_sources.reserve(kvmem_prefetch_.nvme_reads.size());
            for (const KvMemPrefetchNvmeRead &read :
                 kvmem_prefetch_.nvme_reads) {
                admission_sources.push_back(
                    CpuCacheAdmissionSource{
                        read.block_id, read.buffer.data(), read.bytes});
            }
            admit_ssd_sources_to_cpu(admission_sources);
            const uint64_t t_nvme_h2d0 =
                perf_trace ? kvmem_steady_ns() : 0;
...
void QwenExecutor::kvmem_finish_proactive_d2h(bool wait_all) {
    ...
    while (!kvmem_proactive_d2h_batches_.empty()) {
        ...
        for (size_t i = 0; i < batch.block_ids.size(); ++i) {
            ...
            const bool admit_cpu_copy = block_store_ != nullptr;
...
void QwenExecutor::kvmem_prefill_writeback(uint32_t completed_pos) {
    ...
    const uint64_t slab_target_bytes = cfg.optimize_pack
        ? kvmem_io_slab_bytes()
        : std::max<uint64_t>(1, cfg.estimated_block_bytes);
    const uint64_t page_bytes = kvmem_kv_page_bytes();
    size_t candidate = 0;
...
void QwenExecutor::kvmem_stage_out_packed(
        const std::vector<uint32_t> &block_ids) {
    if (!block_store_ || block_ids.empty()) return;
    if (!kvmem_cpu_tier_ && !kvmem_nvme_tier_) {
        throw std::runtime_error(
            "KVMem packed stage-out requires a lower spill tier");
    }
    struct PackedStageOutTask {
        uint32_t block_id = 0;
        int32_t cpu_slot = -1;
        int32_t nvme_slot = -1;
        uint64_t offset = 0;
    ...
    struct PackedStageOutBatch {
        std::unique_ptr<HostBuffer> buffer;
        std::vector<PackedStageOutTask> tasks;
    ...
    const uint64_t slab_target_bytes = kvmem_io_slab_bytes();
    const auto &blocks = block_store_->blocks();
    std::deque<PackedStageOutBatch> pending;
    PackedStageOutBatch building;
    ...
    auto finish_front = [&]() {
        ...
        PackedStageOutBatch batch = std::move(pending.front());
        pending.pop_front();
        const uint64_t wait0 =
            perf_trace ? kvmem_steady_ns() : 0;
        require_status(
        ...
        std::vector<size_t> cpu_task_indices;
        std::vector<uint8_t *> cpu_dsts;
        std::vector<NvmeIoSpan> nvme_spans;
        cpu_task_indices.reserve(batch.tasks.size());
        cpu_dsts.reserve(batch.tasks.size());
        nvme_spans.reserve(batch.tasks.size());
        for (size_t i = 0; i < batch.tasks.size(); ++i) {
            const PackedStageOutTask &task = batch.tasks[i];
            if (task.cpu_slot >= 0) {
                uint8_t *dst =
                    kvmem_cpu_slot_data(task.cpu_slot);
                if (!dst) {
                    throw std::runtime_error(
                        "KVMem packed stage-out lost its CPU slot");
                }
                cpu_task_indices.push_back(i);
                cpu_dsts.push_back(dst);
            } else if (task.nvme_slot >= 0) {
                nvme_spans.push_back(
                    NvmeIoSpan{
                        task.nvme_slot, task.offset, task.bytes});
            } else {
                throw std::runtime_error(
                    "KVMem packed stage-out task has no destination");
            }
        }

        const uint64_t scatter0 =
            perf_trace ? kvmem_steady_ns() : 0;
        auto scatter_task = [&](size_t cpu_index) {
            const PackedStageOutTask &task =
                batch.tasks[cpu_task_indices[cpu_index]];
            std::memcpy(
                cpu_dsts[cpu_index],
                slab + task.offset,
        ...
        const size_t scatter_workers = std::min(
            kvmem_cpu_stageout_threads(), cpu_task_indices.size());
        if (scatter_workers <= 1) {
            for (size_t i = 0; i < cpu_task_indices.size(); ++i) {
                scatter_task(i);
            }
        } else if (kvmem_cpu_worker_pool_) {
            kvmem_cpu_worker_pool_->run(
                cpu_task_indices.size(), scatter_workers, scatter_task);
        } else {
            ...
            auto worker = [&]() {
                for (;;) {
                    const size_t i =
                        next_task.fetch_add(
                            1, std::memory_order_relaxed);
                    if (i >= cpu_task_indices.size()) break;
                    scatter_task(i);
                }
            };
            std::vector<std::thread> workers;
        ...
        if (!nvme_spans.empty()) {
            if (!kvmem_nvme_tier_) {
                throw std::runtime_error(
                    "KVMem packed stage-out has SSD tasks without an "
                    "NVMe tier");
            }
            const uint64_t nvme0 =
                perf_trace ? kvmem_steady_ns() : 0;
            NvmeBatchIoStats stats;
            kvmem_nvme_tier_->write_spans(
                nvme_spans, slab, batch.bytes, &stats);
            if (perf_trace) {
                kvmem_last_stage_out_perf_.nvme_write_ns +=
                    kvmem_steady_ns() - nvme0;
                kvmem_last_stage_out_perf_.async_submitted_bytes +=
                    stats.bytes;
            }
        }
        ...
        for (const PackedStageOutTask &task : batch.tasks) {
        ...
        for (const PackedStageOutTask &task : batch.tasks) {
            ...
            const uint32_t released_pages =
                last_page - first_page + 1;
            const bool to_cpu = task.cpu_slot >= 0;
            if (to_cpu) {
                block_store_->set_block_tier(
                ...
                block_store_->set_block_ssd_backing(
                    task.block_id, task.nvme_slot, true);
                block_store_->set_block_tier(
                    task.block_id, KvTier::SSD,
                    -1, task.nvme_slot);
            }
            ++staged_out;
            if (perf_trace) {
            ...
            if (kvmem_tier_trace_enabled()) {
                std::fprintf(
                    stderr,
                    "[kvmem-tier] stage_out block=%u to=%s "
                    "slot=%d bytes=%llu pages=%u mode=packed\n",
                    task.block_id, to_cpu ? "cpu" : "nvme",
                    to_cpu ? task.cpu_slot : task.nvme_slot,
    ...
    auto submit_building = [&]() {
        ...
        if (packed_targets.empty()) {
            throw std::runtime_error(
                "KVMem packed stage-out has no source tensors");
        }

        uint32_t pages_per_target = 0;
        for (const PackedStageOutTask &task : building.tasks) {
            const KvMemBlock &block = blocks[task.block_id];
            const uint32_t page_size = kv_pages_.page_size;
            const uint32_t first_page =
                block.orig_pos_start / page_size;
        ...
        for (size_t t = 0; t < packed_targets.size(); ++t) {
            ...
            for (const PackedStageOutTask &task : building.tasks) {
                ...
                if (task.offset % page_bytes != 0) {
                    throw std::runtime_error(
                        "KVMem packed stage-out block is not "
                        "page-aligned");
                }
                const uint64_t packed_dst_base =
    ...
    auto place_cpu = [&](uint32_t block_id,
        ...
        if (kvmem_sparse_cpu_tier_) {
            if (kvmem_cpu_budget_has(bytes)) {
                ...
                kvmem_nvme_tier_ &&
                block_store_->config().optimize_stage_in &&
                building.tasks.empty() && pending.empty()) {
                placement =
            ...
            building.tasks.empty() && pending.empty()) {
            placement =
                kvmem_cpu_tier_->place_block_evicting(block_id);
        } else {
            placement =
                kvmem_cpu_tier_->place_block(block_id);
        }
        if (placement.slot < 0 && !kvmem_nvme_tier_) {
            throw std::runtime_error(
                "kvmem: CPU-only packed stage-out cannot reserve a spill "
                "slot; raise --kvmem-cpu-gb or lower --ctx");
        }
        if (placement.slot < 0) return -1;
        if (placement.evicted_block >= 0) {
            if (!kvmem_nvme_tier_) {
                throw std::runtime_error(
                    "KVMem packed CPU eviction has no NVMe tier");
            }
            const uint32_t victim =
                static_cast<uint32_t>(placement.evicted_block);
            if (victim >= blocks.size()) {
                throw std::runtime_error(
                    "KVMem packed CPU eviction selected an unknown block");
            }
            const uint8_t *victim_src =
                kvmem_cpu_slot_data(placement.slot);
            if (!victim_src) {
                throw std::runtime_error(
                    "KVMem packed CPU eviction lost its slot data");
            }
            const KvMemBlock &victim_block = blocks[victim];
            const uint64_t victim_bytes =
                kvmem_block_spill_bytes(victim_block);
            const bool victim_backed =
                block_store_->config().optimize_stage_out &&
                (victim_block.ssd_clean || victim_block.in_flight) &&
                victim_block.nvme_slot >= 0 &&
    ...
    try {
        for (uint32_t block_id : block_ids) {
            ...
            const int32_t cached = kvmem_cpu_tier_
            ...
            if (block_store_->config().optimize_stage_out &&
                kvmem_nvme_tier_ && block.ssd_clean &&
                block.nvme_slot >= 0 &&
                kvmem_nvme_tier_->block_slot(block_id) ==
                    block.nvme_slot) {
                const uint32_t released_pages =
                    release_gpu_pages(block);
                block_store_->set_block_tier(
                    block_id, KvTier::SSD, -1,
                    block.nvme_slot);
                ++staged_out;
                if (perf_trace) {
                    ++kvmem_last_stage_out_perf_.blocks;
                ...
                if (kvmem_tier_trace_enabled()) {
                    std::fprintf(
                        stderr,
                        "[kvmem-tier] stage_out_clean block=%u "
                        "to=nvme nvme_slot=%d avoided_bytes=%llu "
                        "pages=%u\n",
                        block_id, block.nvme_slot,
                        static_cast<unsigned long long>(bytes),
                        released_pages);
                }
                continue;
            ...
            if (cpu_slot < 0) {
                ...
                if (placement.slot < 0) {
                    throw std::runtime_error(
                        "KVMem packed stage-out exhausted the NVMe tier");
                }
                nvme_slot = placement.slot;
            }
...
void QwenExecutor::kvmem_stage_out(const std::vector<uint32_t> &block_ids) {
    ...
    const uint64_t write_slab_target_bytes =
        block_store_->config().optimize_pack
            ? kvmem_io_slab_bytes()
            : std::max<uint64_t>(
                  1, block_store_->config().estimated_block_bytes);
    ...
    auto queue_ssd_write = [&](uint32_t block_id, const uint8_t *src,
        ...
        if (!kvmem_nvme_tier_) {
            throw std::runtime_error(
                "KVMem async stage-out has no NVMe tier");
        }
        const NvmeSlotPlacement p =
            kvmem_nvme_tier_->place_block(block_id);
        if (p.slot < 0) {
            throw std::runtime_error(
                "KVMem inclusive NVMe tier is full");
        }
    ...
    for (uint32_t block_id : block_ids) {
        ...
        if (inclusive && (block.ssd_clean || block.in_flight)) {
            if (!kvmem_nvme_tier_ || block.nvme_slot < 0 ||
                ...
                throw std::runtime_error(
                    "KVMem block has clean/pending SSD backing without "
...
uint32_t QwenExecutor::kvmem_prepare_reselect() {
        ...
        case KvMemMethod::Retrieval:
            ...
            if (kvmem_qc_deltanet_) {
                ...
                const bool key_direction_adaptive =
                    block_store_->config().prototype_mode ==
                    KvMemPrototypeMode::KeyDirectionAdaptive;
                const bool contiguous_subblocks =
                    block_store_->config().retrieval_method ==
                        KvMemRetrievalMethod::SubBlockMeanK &&
                    block_store_->config().prototype_mode ==
                        KvMemPrototypeMode::Contiguous &&
                    kvmem_qc_n_subblocks_ > 1;
                scorer_requested = kvmem_qc_pertoken_
                    ...
                    : contiguous_subblocks
                        ? (block_store_->config().subblock_reduce ==
                                   KvMemSubblockReduce::Sum
                               ? "sub-block-mean-k-sum"
                               : "sub-block-mean-k-max")
                        : "mean-k";
                std::string reason;
    ...
    if (perf_trace || std::getenv("QW3_KVMEM_TRACE")) {
        std::fprintf(
            ...
            block_store_->config().optimize_stage_in ? 1 : 0,
    ...
    const bool heat_aware_cpu_cache = store_cfg.optimize_stage_in;
    ...
    if (method == KvMemMethod::Retrieval) {
        ...
        // PERF_TRACE needs the requested/used scorer and selected-block count
        // for correctness auditing, but must not require the much noisier
        // QW3_KVMEM_TRACE.  The latter also prints every raw-K refresh batch;
        // enabling it during a pack ablation makes logging volume depend on
        // the arm and therefore perturbs the timing being measured.
        if (fallback || perf_trace || std::getenv("QW3_KVMEM_TRACE")) {
            ...
            if (std::strcmp(scorer_used, "per-token") == 0 ||
                ...
                indexed_blocks = kvmem_qc_total_blocks_;
                       ...
                       std::strcmp(
                           scorer_used,
                           "sub-block-mean-k-max") == 0 ||
                       std::strcmp(
                           scorer_used,
                           "sub-block-mean-k-sum") == 0 ||
                       std::strcmp(
                           scorer_used,
...
uint32_t QwenExecutor::kvmem_finish_reselect() {
    ...
    if (perf_trace) {
        ...
        std::fprintf(
            ...
            "cpu_cache_admit_ms=%.3f cpu_cache_admit_blocks=%u "
            "cpu_cache_admit_gib=%.3f "
            ...
            in.h2d_wait_ns * ns_to_ms,
            in.cpu_cache_admit_ns * ns_to_ms,
            in.cpu_cache_admit_blocks,
            in.cpu_cache_admit_bytes * bytes_to_gib,
            in.cpu_blocks,
        ...
        if (kvmem_cpu_tier_) {
            ...
            std::fprintf(
                stderr,
                "[kvmem-cache] stage_in_opt=%d kind=semantic retained=%zu "
...
uint32_t QwenExecutor::kvmem_set_selection(
    ...
    const uint64_t t_select0 =
        perf_trace ? kvmem_steady_ns() : 0;
    KvMemPlan plan = block_store_->set_selection(
        block_ids, force_raw_refresh);
...
bool QwenExecutor::kvmem_retrieval_score(std::string *failure_reason) {
    ...
    if (!g_kbar_) {
        return scorer_unavailable(
            failure_reason, "single_index_not_resident");
    }
    const QwenConfig &cfg = model_.config();
...
void QwenExecutor::kvmem_set_query_span(uint32_t begin, uint32_t end,
    ...
    if (kvmem_qc_n_subblocks_ > 1 && std::getenv("QW3_KVMEM_TRACE")) {
        ...
        if (key_direction_adaptive) {
            ...
            std::fprintf(
                ...
                "slice_tokens=32 prototypes_per_slice=4 "
                "n_slices=%u n_prototypes=%u block_tokens=%u reduce=%s\n",
                bt / 32u,
                kvmem_qc_n_subblocks_, bt,
                kvmem_qc_subblock_max_ ? "max" : "sum");
        } else {
            std::fprintf(
                stderr,
                "[bs-subblock] prototype_mode=contiguous n_subblocks=%u "
                "sub_tokens=%u block_tokens=%u reduce=%s\n",
                kvmem_qc_n_subblocks_,
                (bt + kvmem_qc_n_subblocks_ - 1) /
                    kvmem_qc_n_subblocks_,
                bt, kvmem_qc_subblock_max_ ? "max" : "sum");
        }
    }
    const uint32_t n_kv_heads = cfg.n_kv_heads;
    const uint32_t head_dim = cfg.head_dim;
    ...
    if (kvmem_adaptive_prototypes()) {
        ...
        const uint32_t elem_size =
            dtype && std::strcmp(dtype, "fp32") == 0
                ? sizeof(float)
                : sizeof(uint16_t);
        const uint64_t wanted_bytes =
            per_layer * L * static_cast<uint64_t>(elem_size);
        freshly_allocated =
            !g_kbar_multi_host_ ||
            g_kbar_multi_host_elem_size_ != elem_size ||
            g_kbar_multi_host_capacity_bytes_ < wanted_bytes;
        g_kbar_multi_capacity_ = stride_blocks;
        // This reset is the actual VRAM saving: the complete index authority is
        // pageable host memory; only two fixed-size staging tiles remain on GPU.
        g_kbar_multi_.reset();
        kvmem_prepare_cpu_mean_index(
            per_layer * L, L, stride_blocks, preserve_content_index);
    } else if (!g_kbar_multi_ ||
               stride_blocks > g_kbar_multi_capacity_ ||
               g_kbar_multi_->count < per_layer * L) {
        g_kbar_multi_host_.reset();
        g_kbar_multi_host_capacity_bytes_ = 0;
    ...
    if (total_blocks > g_kbar_global_capacity_) {
        ...
        // A previous GPU-placement run may have left this single-layer fallback
        // allocation live even though CPU mean-K cannot use it.
        g_kbar_.reset();
    }
    if (!g_score_dev_ || g_score_dev_->count < total_blocks) {
        g_score_dev_ =
            backend_.tensor_f32(total_blocks, "g_score");
    }
    if (!g_query_content_) {
        g_query_content_ = backend_.tensor_f32(
            static_cast<uint64_t>(cfg.n_heads) * head_dim, "g_query_content");
...
void QwenExecutor::kvmem_set_retrieval_group_spans(
    ...
    kvmem_round_group_scores_.clear();
    kvmem_round_group_begin_host_.clear();
    kvmem_round_group_end_host_.clear();
    if (!block_store_ ||
        block_store_->config().semantic_expansion ==
            KvMemSemanticExpansion::None) {
        if (!spans.empty()) {
            throw std::runtime_error(
                "KVMem retrieval groups require semantic expansion");
        }
        return;
    }
    uint32_t previous_begin = 0;
    uint32_t previous_end = 0;
    kvmem_retrieval_group_spans_.reserve(spans.size());
    for (size_t i = 0; i < spans.size(); ++i) {
        const auto &[begin, end] = spans[i];
        if (end <= begin ||
            (i > 0 &&
             (begin < previous_begin || end < previous_end))) {
            throw std::runtime_error(
                "KVMem retrieval groups must be sorted and non-empty");
        }
        kvmem_retrieval_group_spans_.emplace_back(begin, end);
        previous_begin = begin;
        previous_end = end;
    }
    if (!spans.empty() && std::getenv("QW3_KVMEM_TRACE")) {
        std::fprintf(stderr,
                     "[bs-semantic-groups] configured groups=%zu "
                     "token_span=[%u,%u)\n",
                     spans.size(), spans.front().first, spans.back().second);
    }
}

bool QwenExecutor::kvmem_prepare_round_score_groups(uint32_t n_blocks) {
    if (!block_store_ ||
        block_store_->config().semantic_expansion ==
            KvMemSemanticExpansion::None ||
        kvmem_retrieval_group_spans_.empty() || n_blocks == 0) {
        return false;
    }
    const KvMemStoreConfig &cfg = block_store_->config();
    const uint32_t bt = std::max<uint32_t>(1, cfg.block_tokens);
    const uint32_t ns = std::max<uint32_t>(1, kvmem_qc_n_subblocks_);
    if (bt % ns != 0) {
        throw std::runtime_error(
            "KVMem semantic retrieval requires block_tokens to be divisible "
            "by subblocks");
    }
    const uint32_t sub_tokens = bt / ns;
    const uint64_t total_subblocks =
        static_cast<uint64_t>(n_blocks) * ns;
    kvmem_round_group_begin_host_.clear();
    kvmem_round_group_end_host_.clear();
    kvmem_round_group_begin_host_.reserve(
        kvmem_retrieval_group_spans_.size());
    kvmem_round_group_end_host_.reserve(
        kvmem_retrieval_group_spans_.size());
    for (const auto &[begin, end] : kvmem_retrieval_group_spans_) {
        // Include every scoring sub-block that overlaps the exact group. At a
        // non-32-token boundary the same sub-block can contribute to two
        // adjacent group scores; physical selection later deduplicates its
        // containing block. The ambiguity is bounded to one sub-block.
        const uint64_t first = begin / sub_tokens;
        const uint64_t last_exclusive =
            (static_cast<uint64_t>(end) + sub_tokens - 1) / sub_tokens;
        const uint64_t capped_first =
            std::min(first, total_subblocks);
        const uint64_t capped_end =
            std::min(last_exclusive, total_subblocks);
        kvmem_round_group_begin_host_.push_back(
            static_cast<int32_t>(capped_first));
        kvmem_round_group_end_host_.push_back(
            static_cast<int32_t>(capped_end));
    }
    const uint32_t ng = static_cast<uint32_t>(
        kvmem_round_group_begin_host_.size());
    if (ng == 0) return false;
    if (!kvmem_round_group_begin_dev_ ||
        kvmem_round_group_capacity_ < ng) {
        kvmem_round_group_capacity_ = ng;
        kvmem_round_group_begin_dev_ =
            backend_.tensor_i32(ng, "kvmem_round_group_begin");
        kvmem_round_group_end_dev_ =
            backend_.tensor_i32(ng, "kvmem_round_group_end");
        kvmem_group_score_dev_ =
            backend_.tensor_f32(ng, "kvmem_semantic_group_score");
    }
    require_status(backend_.copy_i32_from_host(
        *kvmem_round_group_begin_dev_, 0,
        kvmem_round_group_begin_host_.data(), ng));
    require_status(backend_.copy_i32_from_host(
        *kvmem_round_group_end_dev_, 0,
        kvmem_round_group_end_host_.data(), ng));
    return true;
}
...
std::vector<uint32_t> QwenExecutor::kvmem_selection_with_pin() {
    ...
    if (round_ready) {
        return block_store_->pick_semantic_groups(
            kvmem_retrieval_group_spans_,
            kvmem_round_group_scores_, mandatory);
    }
...
void QwenExecutor::kvmem_capture_kbar_multi(uint32_t slot, uint32_t batch,
    ...
    if (!kvmem_qc_capture_active_) return;
    const DeviceTensor *k_src = kvmem_index_rebuild_source_ != nullptr
                                    ? kvmem_index_rebuild_source_
                                    : k_batch_.get();
    if (!kvmem_mean_index_available() || !k_src || g_kbar_multi_ready_) {
        return;
    }
    if (kvmem_qc_total_blocks_ == 0 || !block_store_) return;
    const QwenConfig &cfg = model_.config();
    // Archived raw K is stored position-free, so the builders must not de-RoPE
    // it again. rope_dim == 0 makes their de-rotate an exact identity.
    const uint32_t derope_dim =
        kvmem_index_rebuild_source_ != nullptr ? 0u : cfg.rope_dim;
    const uint32_t n_kv_heads = cfg.n_kv_heads;
    const uint32_t head_dim = cfg.head_dim;
    const uint32_t bt = std::max<uint32_t>(block_store_->config().block_tokens, 1u);
    ...
    if (kvmem_mean_index_cpu() && !kvmem_adaptive_prototypes()) {
        ...
        } else if (g_kbar_capture_active_slot_ < 0) {
            throw std::runtime_error(
                "KVMem CPU mean-index capture has no active staging slot");
        }
        if (cpu_stage_slot == UINT32_MAX) {
            cpu_stage_slot =
                static_cast<uint32_t>(g_kbar_capture_active_slot_);
        }
        capture_index =
            g_kbar_stream_slots_[cpu_stage_slot].device.get();
        // Staging is packed densely by this chunk, independently of the fixed
        // canonical host stride.
        kbar_block_base =
    ...
    if (key_direction_adaptive && off == 0) {
        ...
        throw std::runtime_error(
            "key-direction-fixed4 mean-K capture requires a block-aligned "
            "prefill suffix");
    } else if (off == 0) {
    ...
    if (kvmem_qc_pertoken_ && g_kraw_multi_ &&
        ...
        (void)backend_.derope_store_content_batch_device(
            *k_src, *g_kraw_multi_, out_base_elem, k_token_stride, store_rows,
            n_kv_heads, head_dim, derope_dim,
            static_cast<int32_t>(rope_base_pos), cfg.rope_theta);
    }
...
void QwenExecutor::kvmem_capture_decode_block(uint32_t true_block_index,
                                              uint32_t rows) {
    if (!g_kbar_decode_stage_ ||
        !kvmem_mean_index_available() || rows == 0) {
        return;
    }
...
bool QwenExecutor::kvmem_score_cpu_mean_index(
        uint32_t n_blocks, uint32_t score_query_tokens,
    ...
    if (!kvmem_mean_index_cpu() ||
        ...
        g_kbar_stream_tile_blocks_ == 0) {
        return scorer_unavailable(
            failure_reason, "cpu_mean_index_stream_unavailable");
    }
    kvmem_drain_mean_index_capture();
    const QwenConfig &cfg = model_.config();
    ...
    const uint64_t distributions64 =
        ...
        score_query_tokens * cfg.n_heads;
    if (distributions64 == 0 || distributions64 > UINT32_MAX) {
        return scorer_unavailable(
            failure_reason, "cpu_mean_index_distribution_overflow");
    }
    ...
    if (!g_kbar_stream_global_max_ ||
        ...
        g_kbar_stream_global_max_ =
            backend_.tensor_f32(
                distributions, "g_kbar_stream_global_max");
        g_kbar_stream_global_sum_ =
            backend_.tensor_f32(
                distributions, "g_kbar_stream_global_sum");
        g_kbar_stream_distribution_capacity_ = distributions;
    }
    const uint32_t n_groups = round_mode
        ? static_cast<uint32_t>(
              kvmem_retrieval_group_spans_.size())
        : 0;
    if (round_mode) {
        const uint64_t group_dist_count =
            static_cast<uint64_t>(n_groups) * distributions;
        if (group_dist_count == 0 ||
            group_dist_count > SIZE_MAX / sizeof(float)) {
            return scorer_unavailable(
                failure_reason,
                "cpu_mean_index_group_workspace_overflow");
        }
        if (!g_kbar_stream_group_dist_ ||
            g_kbar_stream_group_dist_capacity_ <
                group_dist_count) {
            g_kbar_stream_group_dist_ =
                backend_.tensor_f32(
                    group_dist_count,
                    "g_kbar_stream_group_dist");
            g_kbar_stream_group_dist_capacity_ =
                group_dist_count;
        }
        require_status(
            backend_.zero_tensor(*g_kbar_stream_group_dist_));
    }

    const uint32_t tile_capacity =
        g_kbar_stream_tile_blocks_;
    const uint32_t tiles =
        (n_blocks + tile_capacity - 1) / tile_capacity;
    const uint64_t per_block_bytes =
        static_cast<uint64_t>(layers) * ns *
        cfg.n_kv_heads * cfg.head_dim *
        g_kbar_multi_host_elem_size_;
    ...
    auto run_pass = [&](uint32_t pass) -> bool {
        auto submit = [&](uint32_t tile_index) {
            const uint32_t slot_index =
                tile_index %
                static_cast<uint32_t>(
                    g_kbar_stream_slots_.size());
            MeanIndexStageSlot &slot =
                g_kbar_stream_slots_[slot_index];
            ...
            const uint32_t first =
                tile_index * tile_capacity;
            const uint32_t count =
                std::min(tile_capacity, n_blocks - first);
            const uint64_t pack_begin = kvmem_steady_ns();
            kvmem_pack_mean_index_tile(first, count, slot);
            packed_ns += kvmem_steady_ns() - pack_begin;
            const uint64_t bytes =
        ...
        for (uint32_t tile = 0; tile < tiles; ++tile) {
            ...
            DeviceStatus st;
            if (pass == 1) {
                st = backend_.block_attn_stream_lse_device(
                    *g_kbar_stream_global_max_,
                    *g_kbar_stream_global_sum_,
                    *g_query_multi_, *slot.device,
                    layers, score_query_tokens,
                    q_layer_stride, count,
                    /*kbar_layer_stride=*/count,
                    /*global_block_base=*/first,
                    cfg.n_heads, cfg.n_kv_heads,
                    cfg.head_dim, scale,
                    excl_lo_end, excl_hi_begin, ns,
                    /*initialize=*/tile == 0 ? 1u : 0u);
            } else if (round_mode) {
                st =
                    backend_.
                        block_attn_stream_group_update_device(
                            *g_kbar_stream_group_dist_,
                            *g_query_multi_, *slot.device,
                            *g_kbar_stream_global_max_,
                            *g_kbar_stream_global_sum_,
                            ...
                            *kvmem_round_group_end_dev_,
                            ...
                            block_store_->config().
                                    ...
                                    KvMemGroupScoreReduce::
                                        LengthNormalizedMass
                                ? 1u
                                : 0u);
            } else {
                st = backend_.block_attn_stream_score_device(
                    *g_score_dev_, *g_query_multi_,
                    *slot.device,
                    *g_kbar_stream_global_max_,
                    *g_kbar_stream_global_sum_,
            ...
            if (!st.ok) {
                return scorer_backend_unavailable(
                    failure_reason,
                    pass == 1
                        ? "cpu_mean_index_lse_failed"
                        : "cpu_mean_index_score_failed",
                    st);
            }
            require_status(
                backend_.record_execution_fence(
                    slot.compute_done));
            if (tile + 2 < tiles) submit(tile + 2);
        }
        require_status(backend_.synchronize());
        return true;
    };
    ...
    if (round_mode) {
        const float distribution_weight =
            ...
            (static_cast<float>(layers) *
             static_cast<float>(cfg.n_heads));
        if (auto st =
                backend_.
                    block_attn_stream_group_finalize_device(
                        *kvmem_group_score_dev_,
                        *g_kbar_stream_group_dist_,
                        n_groups, distributions,
                        distribution_weight, accumulate);
            !st.ok) {
            return scorer_backend_unavailable(
                failure_reason,
                "cpu_mean_index_group_finalize_failed", st);
        }
    }
    if (std::getenv("QW3_KVMEM_TRACE") ||
        ...
        std::fprintf(
            stderr,
            "[bs-mean-stream] placement=cpu exact=1 "
            "blocks=%u subblocks=%u layers=%u query_tokens=%u "
            "tiles=%u tile_blocks=%u transferred_mib=%.2f "
            "pack_ms=%.3f submit_total_ms=%.3f groups=%u\n",
            n_blocks, ns, layers, score_query_tokens,
            tiles, tile_capacity,
...
uint64_t QwenExecutor::kvmem_adaptive_layer_value_bytes(
        uint32_t layer) const {
    if (layer < g_adaptive_layer_values_pinned_.size() &&
        g_adaptive_layer_values_pinned_[layer]) {
        return g_adaptive_layer_values_pinned_size_[layer];
    }
    return layer < g_adaptive_layer_values_host_.size()
        ? g_adaptive_layer_values_host_[layer].size()
        : 0;
}

const uint8_t *QwenExecutor::kvmem_adaptive_layer_value_data(
        uint32_t layer) const {
    if (layer < g_adaptive_layer_values_pinned_.size() &&
        g_adaptive_layer_values_pinned_[layer]) {
        return static_cast<const uint8_t *>(
            g_adaptive_layer_values_pinned_[layer]->data);
    }
    return layer < g_adaptive_layer_values_host_.size()
        ? g_adaptive_layer_values_host_[layer].data()
        : nullptr;
}

uint8_t *QwenExecutor::kvmem_adaptive_layer_value_data(uint32_t layer) {
    return const_cast<uint8_t *>(
        static_cast<const QwenExecutor *>(this)
            ->kvmem_adaptive_layer_value_data(layer));
}

void QwenExecutor::kvmem_migrate_adaptive_index_to_pinned() {
    if (!kvmem_adaptive_prototypes() || !kvmem_mean_index_cpu() ||
        !env_flag_enabled("QW3_KVMEM_ADAPTIVE_PINNED_INDEX", true)) {
        return;
    }
    const uint32_t layers = static_cast<uint32_t>(
        g_adaptive_layer_values_host_.size());
    if (layers == 0) return;
    g_adaptive_layer_values_pinned_.resize(layers);
    g_adaptive_layer_values_pinned_size_.resize(layers, 0);
    g_adaptive_layer_values_pinned_capacity_.resize(layers, 0);
    const uint64_t reserve_bytes =
        static_cast<uint64_t>(std::clamp<uint32_t>(
            env_uint32_or("QW3_KVMEM_ADAPTIVE_PINNED_RESERVE_MIB", 8),
            1u, 64u)) * 1024ull * 1024ull;
    uint64_t migrated_bytes = 0;
    const uint64_t begin_ns = kvmem_steady_ns();
    for (uint32_t layer = 0; layer < layers; ++layer) {
        auto &source = g_adaptive_layer_values_host_[layer];
        if (source.empty() || g_adaptive_layer_values_pinned_[layer]) {
            continue;
        }
        if (source.size() > SIZE_MAX - reserve_bytes) {
            break;
        }
        const uint64_t capacity = source.size() + reserve_bytes;
        std::unique_ptr<HostBuffer> pinned;
        try {
            pinned = backend_.host_buffer(
                capacity, "g_adaptive_layer_values_pinned");
        } catch (const std::exception &e) {
            std::fprintf(
                stderr,
                "[bs-adaptive-index] pinned_degraded=1 layer=%u "
                "reason=%s action=pageable-fallback\n",
                layer, e.what());
            break;
        }
        std::memcpy(pinned->data, source.data(), source.size());
        migrated_bytes += source.size();
        g_adaptive_layer_values_pinned_size_[layer] = source.size();
        g_adaptive_layer_values_pinned_capacity_[layer] = capacity;
        g_adaptive_layer_values_pinned_[layer] = std::move(pinned);
        // Release this layer before allocating the next one, bounding the
        // migration's transient duplicate footprint to one layer.
        std::vector<uint8_t>().swap(source);
    }
    uint32_t pinned_layers = 0;
    for (const auto &layer : g_adaptive_layer_values_pinned_) {
        pinned_layers += layer ? 1u : 0u;
    }
    std::fprintf(
        stderr,
        "[bs-adaptive-index] pinned_authority=1 layers=%u/%u "
        "bytes=%llu gib=%.3f reserve_mib_per_layer=%llu migrate_ms=%.3f\n",
        pinned_layers, layers,
        static_cast<unsigned long long>(migrated_bytes),
        static_cast<double>(migrated_bytes) /
            (1024.0 * 1024.0 * 1024.0),
...
void QwenExecutor::kvmem_reset_adaptive_index(
    ...
    if (!preserve_content_index || !compatible) {
        ...
        kvmem_drain_adaptive_gpu_uploads();
        g_adaptive_gpu_layer_arenas_.clear();
        for (auto &done : g_adaptive_upload_done_) {
            done.reset();
        }
        g_adaptive_upload_sequence_ = 0;
        g_adaptive_incremental_upload_bytes_ = 0;
...
void QwenExecutor::kvmem_drain_adaptive_gpu_uploads() {
    for (auto &done : g_adaptive_upload_done_) {
        if (!done) continue;
        const uint64_t wait_begin = kvmem_steady_ns();
        require_status(
            backend_.wait_kv_transfer_fence(done));
        g_adaptive_incremental_wait_ns_ +=
            kvmem_steady_ns() - wait_begin;
    }
}

void QwenExecutor::kvmem_upload_adaptive_gpu_layer(
        uint32_t layer, uint32_t first_prototype) {
    if (!kvmem_adaptive_prototypes() ||
        kvmem_mean_index_cpu() || !block_store_ ||
        env_flag_enabled(
            "QW3_KVMEM_ADAPTIVE_GPU_PACKED", false)) {
        return;
    }
    const QwenConfig &cfg = model_.config();
    const uint32_t layers = kvmem_qc_num_layers_;
    if (layer >= layers ||
        layer >= g_adaptive_layer_values_host_.size() ||
        layer >= g_adaptive_layer_parent_host_.size()) {
        throw std::runtime_error(
            "KVMem Adaptive incremental GPU layer is out of range");
    }
    if (g_adaptive_gpu_layer_arenas_.size() != layers) {
        kvmem_drain_adaptive_gpu_uploads();
        require_status(backend_.synchronize());
        g_adaptive_gpu_layer_arenas_.clear();
        g_adaptive_gpu_layer_arenas_.resize(layers);
    }
    const uint64_t row_elems =
        static_cast<uint64_t>(cfg.n_kv_heads) * cfg.head_dim;
    ...
    const uint64_t required_rows =
        g_adaptive_layer_parent_host_[layer].size();
    if (row_bytes == 0 ||
        g_adaptive_layer_values_host_[layer].size() !=
            required_rows * row_bytes ||
        required_rows > UINT32_MAX) {
        throw std::runtime_error(
            "KVMem Adaptive incremental GPU layer is inconsistent");
    }
    AdaptiveGpuLayerArena &arena =
        g_adaptive_gpu_layer_arenas_[layer];
    constexpr uint64_t kArenaGrowthBytes =
        64ull * 1024ull * 1024ull;
    const uint64_t growth_rows =
        std::max<uint64_t>(1, kArenaGrowthBytes / row_bytes);
    if (!arena.values ||
        arena.values->elem_size !=
            g_adaptive_index_elem_size_ ||
        arena.capacity_rows < required_rows) {
        kvmem_drain_adaptive_gpu_uploads();
        require_status(backend_.synchronize());
        const uint64_t capacity_rows =
            std::max<uint64_t>(
                growth_rows,
                ((required_rows + growth_rows - 1) /
                 growth_rows) *
                    growth_rows);
        arena.values =
            kvmem_alloc_mean_index_tensor(
                capacity_rows * row_elems,
                "g_adaptive_gpu_layer_values");
        arena.capacity_rows = capacity_rows;
        arena.prototype_count = 0;
        first_prototype = 0;
        // The tensor allocation/zeroing is ordered on the execution stream.
        // Make the transfer stream observe it before the first suffix upload.
        require_status(
            backend_.begin_kv_transfer_from_device());
    }
    if (first_prototype > required_rows) {
        throw std::runtime_error(
            "KVMem Adaptive GPU dirty suffix is out of range");
    }
    if (first_prototype == required_rows) {
        arena.prototype_count =
            static_cast<uint32_t>(required_rows);
        return;
    }

    const uint32_t upload_mib = std::clamp<uint32_t>(
        env_uint32_or(
            "QW3_KVMEM_ADAPTIVE_UPLOAD_MIB", 64),
        8u, 256u);
    const uint64_t upload_bytes =
        static_cast<uint64_t>(upload_mib) *
        1024ull * 1024ull;
    if (g_adaptive_upload_pinned_capacity_ < upload_bytes ||
        ...
        g_adaptive_upload_pinned_[0] =
            backend_.host_buffer(
                upload_bytes,
                "g_adaptive_incremental_upload_0");
        g_adaptive_upload_pinned_[1] =
            backend_.host_buffer(
                upload_bytes,
                "g_adaptive_incremental_upload_1");
        g_adaptive_upload_pinned_capacity_ = upload_bytes;
    }
    const auto &values =
        g_adaptive_layer_values_host_[layer];
    uint64_t source_byte =
        static_cast<uint64_t>(first_prototype) *
        row_bytes;
    const uint64_t end_byte = required_rows * row_bytes;
    while (source_byte < end_byte) {
        const uint32_t slot =
            g_adaptive_upload_sequence_++ & 1u;
        if (g_adaptive_upload_done_[slot]) {
            const uint64_t wait_begin = kvmem_steady_ns();
            require_status(
                backend_.wait_kv_transfer_fence(
                    g_adaptive_upload_done_[slot]));
            g_adaptive_incremental_wait_ns_ +=
                kvmem_steady_ns() - wait_begin;
        }
        const uint64_t bytes =
            std::min<uint64_t>(
                upload_bytes, end_byte - source_byte);
        const uint64_t pack_begin = kvmem_steady_ns();
        std::memcpy(
            g_adaptive_upload_pinned_[slot]->data,
            values.data() + source_byte,
            static_cast<size_t>(bytes));
        g_adaptive_incremental_pack_ns_ +=
            kvmem_steady_ns() - pack_begin;
        require_status(
            backend_.begin_kv_transfer_to_device());
        require_status(
            backend_.copy_bytes_from_host_async(
                *arena.values, source_byte,
                g_adaptive_upload_pinned_[slot]->data,
                bytes));
        require_status(
            backend_.record_kv_transfer_fence(
                g_adaptive_upload_done_[slot]));
        g_adaptive_incremental_upload_bytes_ += bytes;
        source_byte += bytes;
    }
    arena.prototype_count =
        static_cast<uint32_t>(required_rows);
...
void QwenExecutor::kvmem_capture_adaptive_layer(
    ...
    KvmemThreadAffinityScope numa_scope(kvmem_numa_cpus_);
    const QwenConfig &cfg = model_.config();
    // Archived raw K is already content-frame; see kvmem_capture_kbar_multi.
    const uint32_t derope_dim =
        kvmem_index_rebuild_source_ != nullptr ? 0u : cfg.rope_dim;
    const uint32_t layers = kvmem_qc_num_layers_;
    const uint32_t bt =
    ...
    auto &pageable_values = g_adaptive_layer_values_host_[slot];
    const bool pinned_values =
        slot < g_adaptive_layer_values_pinned_.size() &&
        g_adaptive_layer_values_pinned_[slot];
    ...
    if (truncate_proto < parents.size()) {
        ...
        const uint64_t truncated_bytes =
            static_cast<uint64_t>(truncate_proto) * proto_row_bytes;
        if (pinned_values) {
            g_adaptive_layer_values_pinned_size_[slot] =
                truncated_bytes;
        } else {
            pageable_values.resize(
                static_cast<size_t>(truncated_bytes));
        }
        ...
        for (uint32_t slice = 0; slice < n_slices; ++slice) {
            ...
            for (uint32_t p = 0; p < keep; ++p) {
                ...
                const uint64_t old_size =
                    kvmem_adaptive_layer_value_bytes(slot);
                const uint64_t new_size = old_size + proto_row_bytes;
                if (pinned_values) {
                    if (new_size >
                        g_adaptive_layer_values_pinned_capacity_[slot]) {
                        throw std::runtime_error(
                            "KVMem pinned Adaptive replay suffix exceeded "
                            "QW3_KVMEM_ADAPTIVE_PINNED_RESERVE_MIB");
                    }
                    std::memcpy(
                        kvmem_adaptive_layer_value_data(slot) + old_size,
                        dense.data() + src_byte,
                        static_cast<size_t>(proto_row_bytes));
                    g_adaptive_layer_values_pinned_size_[slot] = new_size;
                } else {
                    pageable_values.resize(
                        static_cast<size_t>(new_size));
                    std::memcpy(
                        pageable_values.data() + old_size,
                        dense.data() + src_byte,
                        static_cast<size_t>(proto_row_bytes));
                }
    ...
    kvmem_upload_adaptive_gpu_layer(slot, truncate_proto);
}
...
void QwenExecutor::kvmem_finalize_adaptive_index() {
    ...
    if (incremental_gpu &&
        !g_adaptive_index_dirty_ &&
        g_adaptive_total_prototypes_ > 0 &&
        g_adaptive_gpu_layer_arenas_.size() ==
            kvmem_qc_num_layers_) {
        if (std::getenv("QW3_KVMEM_TRACE")) {
            std::fprintf(
                stderr,
                "[bs-adaptive-index] placement=gpu "
                "layout=per-layer-incremental reuse=1 dirty=0 "
                "prototypes=%u\n",
                g_adaptive_total_prototypes_);
        }
        return;
    ...
    if (kvmem_mean_index_cpu()) {
        for (uint32_t l = 0; l < layers; ++l) {
            const uint64_t layer_count =
                g_adaptive_layer_parent_host_[l].size();
            if (kvmem_adaptive_layer_value_bytes(l) !=
    ...
    if (incremental_gpu) {
        ...
        if (g_adaptive_gpu_layer_arenas_.size() != layers) {
            throw std::runtime_error(
                "KVMem Adaptive incremental GPU arenas are incomplete");
        }
        for (uint32_t l = 0; l < layers; ++l) {
            AdaptiveGpuLayerArena &arena =
            ...
            const uint64_t layer_count =
                g_adaptive_layer_parent_host_[l].size();
            if (!arena.values ||
                arena.prototype_count != layer_count ||
                arena.capacity_rows < layer_count) {
                throw std::runtime_error(
                    "KVMem Adaptive incremental GPU upload is incomplete");
            }
            if (!arena.block_offsets ||
        ...
        if (std::getenv("QW3_KVMEM_TRACE") ||
            ...
            for (const auto &arena :
                ...
                capacity_bytes +=
                    arena.capacity_rows * prototype_row_bytes;
            }
            std::fprintf(
                stderr,
                "[bs-adaptive-index] placement=gpu "
                "layout=per-layer-incremental layers=%u blocks=%u "
                "prototypes=%u avg_p=%.3f p1=%llu p2=%llu p4=%llu "
                "bytes=%llu gib=%.3f capacity_gib=%.3f "
                "incremental_h2d_gib=%.3f pack_ms=%.3f "
                "h2d_wait_ms=%.3f finalize_ms=%.3f\n",
                layers, n_blocks, g_adaptive_total_prototypes_, avg,
                static_cast<unsigned long long>(
                    g_adaptive_p1_slices_),
                static_cast<unsigned long long>(
                ...
                static_cast<double>(capacity_bytes) /
                    (1024.0 * 1024.0 * 1024.0),
                static_cast<double>(
                    g_adaptive_incremental_upload_bytes_) /
                    (1024.0 * 1024.0 * 1024.0),
                g_adaptive_incremental_pack_ns_ / 1.0e6,
                g_adaptive_incremental_wait_ns_ / 1.0e6,
                (kvmem_steady_ns() - finalize_begin_ns) / 1.0e6);
        }
        return;
    }
    ...
    for (uint32_t l = 0; l < layers; ++l) {
        ...
        const uint64_t values_size =
            kvmem_adaptive_layer_value_bytes(l);
        const uint8_t *values =
            kvmem_adaptive_layer_value_data(l);
        if (values_size != layer_count * row_bytes || !values) {
            throw std::runtime_error(
                "KVMem adaptive packed host values are inconsistent");
        }
        for (uint64_t offset = 0; offset < values_size;) {
            ...
            const uint64_t bytes =
                std::min<uint64_t>(
                    upload_bytes, values_size - offset);
            const uint64_t gather_begin = kvmem_steady_ns();
            auto *dst = static_cast<uint8_t *>(
                g_adaptive_upload_pinned_[slot]->data);
            const uint8_t *src = values + offset;
...
bool QwenExecutor::kvmem_score_cpu_adaptive_index_layer_one_pass(
    ...
    const size_t stream_workers = std::min<size_t>(
        kvmem_adaptive_pack_threads(),
        std::max<uint64_t>(
            1, value_bytes / (4ull * 1024ull * 1024ull)));
    ...
    auto submit = [&](uint32_t job_index) {
        ...
        auto *host =
            static_cast<uint8_t *>(slot.host->data);
        const bool direct_pinned =
            job.layer < g_adaptive_layer_values_pinned_.size() &&
            g_adaptive_layer_values_pinned_[job.layer];
        const uint8_t *source =
            kvmem_adaptive_layer_value_data(job.layer) +
        ...
        if (direct_pinned) {
            // Canonical archive bytes are already page-locked; preserve them
            // until the transfer fence completes and avoid the bounce copy.
        } else if (workers <= 1 || !stream_pool) {
            std::memcpy(
                host, source,
...
bool QwenExecutor::kvmem_score_cpu_adaptive_index(
    ...
    const char *tile_scorer_env =
        std::getenv("QW3_KVMEM_ADAPTIVE_TILE_SCORER");
    const bool tile_scorer_scalar =
        tile_scorer_env &&
        (std::strcmp(tile_scorer_env, "scalar") == 0 ||
         std::strcmp(tile_scorer_env, "legacy") == 0);
    bool use_gemm_tile_scorer =
        use_tiled_one_pass && !tile_scorer_scalar &&
        g_query_multi_ &&
        g_query_multi_->dtype == DeviceTensorDType::F16 &&
        !g_adaptive_stream_slots_.empty() &&
        g_adaptive_stream_slots_[0].values &&
        g_adaptive_stream_slots_[0].values->dtype ==
            DeviceTensorDType::F16 &&
        gqa_group >= 1u && gqa_group <= 6u;
    uint64_t gemm_logits_entries = 0;
    if (use_gemm_tile_scorer) {
        const uint64_t capacity_rows_u64 = capacity_rows;
        if (static_cast<uint64_t>(n_tokens) >
            UINT64_MAX / capacity_rows_u64 ||
            static_cast<uint64_t>(n_tokens) * capacity_rows_u64 >
                UINT64_MAX / gqa_group) {
            use_gemm_tile_scorer = false;
        } else {
            gemm_logits_entries =
                static_cast<uint64_t>(n_tokens) * capacity_rows_u64 *
                gqa_group;
    ...
    if (use_gemm_tile_scorer &&
        (!g_adaptive_stream_logits_ ||
         g_adaptive_stream_logits_capacity_ < gemm_logits_entries)) {
        g_adaptive_stream_logits_ = backend_.scratch_f32(
            gemm_logits_entries, "g_adaptive_stream_gemm_logits");
        g_adaptive_stream_logits_capacity_ = gemm_logits_entries;
    }
    ...
    auto run_pass = [&](uint32_t pass) -> bool {
        const uint32_t partial_layers = pass == 0
            ? std::min<uint32_t>(
                  env_uint32_or("QW3_KVMEM_ADAPTIVE_PARTIAL_LAYERS", 0),
                  layers)
            : 0;
        g_adaptive_partial_selection_.clear();
        auto submit = [&](uint32_t tile_index) {
            ...
            auto *host =
                static_cast<uint8_t *>(slot.host->data);
            const bool direct_pinned =
                tile.layer < g_adaptive_layer_values_pinned_.size() &&
                g_adaptive_layer_values_pinned_[tile.layer];
            const uint64_t source_byte =
                static_cast<uint64_t>(tile.prototype_begin) *
                row_bytes;
            const uint8_t *source =
                kvmem_adaptive_layer_value_data(tile.layer) + source_byte;
            const size_t workers = std::min<size_t>(
                stream_workers,
                std::max<uint64_t>(
                    1, tile_value_bytes /
                           (4ull * 1024ull * 1024ull)));
            if (direct_pinned) {
                // The fixed archive authority itself is pinned, so it can be
                // the asynchronous DMA source for this tile.
            } else if (workers <= 1 || !stream_pool) {
                std::memcpy(
                    host, source,
        ...
        for (uint32_t i = 0; i < tiles.size(); ++i) {
            ...
            if (pass == 0) {
                if (use_gemm_tile_scorer) {
                    st = backend_.
                        block_attn_stream_adaptive_block_stats_gemm_device(
                            *g_adaptive_stream_block_max_,
                            *g_adaptive_stream_block_sum_,
                            *g_adaptive_stream_logits_,
                    ...
                    if (!st.ok) {
                        if (std::getenv("QW3_KVMEM_TRACE") ||
                            ...
                            std::fprintf(
                                stderr,
                                "[bs-adaptive-stream-fallback] "
                                "from=tensor-gemm to=scalar-one-dot "
                                "reason=%s\n",
                                st.message ? st.message : "unknown");
                        }
                        use_gemm_tile_scorer = false;
            ...
            if (pass == 0 &&
                ...
                if (partial_layers > 0 &&
                    tile.layer + 1 == partial_layers) {
                    // The per-layer finalize above has committed an exact
                    // prefix-layer score into g_score_dev_. Snapshot only the
                    // small block-score vector; the streamed prototype index
                    // and final computation remain untouched.
                    require_status(backend_.synchronize());
                    std::vector<float> partial(n_blocks, 0.0f);
                    require_status(backend_.copy_to_host(
                        *g_score_dev_, partial.data(), 0, n_blocks));
                    std::vector<double> partial_best(
                        block_store_->block_count(), 0.0);
                    for (uint32_t id = 0;
                         id < n_blocks && id < partial_best.size(); ++id) {
                        partial_best[id] =
                            static_cast<double>(partial[id]);
                    }
                    block_store_->set_retrieval_scores(partial_best);
                    g_adaptive_partial_selection_ =
    ...
    if (use_tiled_one_pass) {
        ...
        if (std::getenv("QW3_KVMEM_TRACE") ||
            ...
            std::fprintf(
                ...
                "gqa_group=%u tile_scorer=%s logits_mib=%.2f "
                "transferred_gib=%.3f "
                ...
                use_gemm_tile_scorer ? "tensor-gemm" : "scalar-one-dot",
                static_cast<double>(
                    gemm_logits_entries * sizeof(float)) /
                    (1024.0 * 1024.0),
                static_cast<double>(transferred_bytes) /
                    (1024.0 * 1024.0 * 1024.0),
                pack_ns / 1.0e6, h2d_wait_ns / 1.0e6,
    ...
    if (std::getenv("QW3_KVMEM_TRACE") ||
        ...
        std::fprintf(
            stderr,
            "[bs-adaptive-stream] placement=cpu exact=1 "
            "mode=tiled-two-pass passes=2 layers=%u blocks=%u query_tokens=%u "
...
bool QwenExecutor::kvmem_score_gpu_adaptive_index_layer_one_pass(
        uint32_t n_blocks, uint32_t n_tokens,
        uint32_t q_layer_stride, float scale,
        uint32_t excl_lo_end, uint32_t excl_hi_begin,
        uint32_t accumulate, std::string *failure_reason) {
    if (!kvmem_adaptive_prototypes() ||
        ...
        return scorer_unavailable(
            failure_reason,
            "gpu_adaptive_layer_index_unavailable");
    }
    const QwenConfig &cfg = model_.config();
    const uint32_t layers = kvmem_qc_num_layers_;
    const uint32_t included_begin =
    ...
    if (layers == 0 || included_begin >= included_end ||
        ...
        return scorer_unavailable(
            failure_reason,
            "gpu_adaptive_layer_index_shape_invalid");
    }
    if (!accumulate) {
        require_status(backend_.zero_tensor(*g_score_dev_));
    }
    ...
    // The original GPU-resident scorer launches a scalar dot-product kernel
    // over an entire layer.  That is acceptable for short queries, but becomes
    // catastrophically slow once Q is a few hundred tokens (the 10M/520-token
    // benchmark spent ~95 s here).  Reuse the exact tiled Tensor-GEMM primitive
    // used by the CPU-offloaded one-dot path.  The only difference is that each
    // prototype tile is copied device-to-device from its persistent layer arena
    // instead of crossing PCIe.  Block-relative max/LSE statistics and the
    // per-layer finalize are identical, so this changes neither scoring nor
    // Top-K semantics.
    const uint32_t gqa_group =
        cfg.n_kv_heads > 0 ? cfg.n_heads / cfg.n_kv_heads : 0;
    const uint64_t row_elems =
        static_cast<uint64_t>(cfg.n_kv_heads) * cfg.head_dim;
    const uint64_t row_bytes = row_elems * g_adaptive_index_elem_size_;
    const bool tensor_gemm_supported =
        env_flag_enabled("QW3_KVMEM_ADAPTIVE_GPU_TENSOR_GEMM", true) &&
        cfg.n_kv_heads > 0 && cfg.n_heads % cfg.n_kv_heads == 0 &&
        gqa_group >= 1u && gqa_group <= 6u && row_bytes > 0 &&
        g_query_multi_->dtype == DeviceTensorDType::F16 &&
        !g_adaptive_gpu_layer_arenas_.empty() &&
        g_adaptive_gpu_layer_arenas_[0].values &&
        g_adaptive_gpu_layer_arenas_[0].values->dtype ==
            DeviceTensorDType::F16;
    if (tensor_gemm_supported) {
        uint64_t max_block_prototypes = 1;
        for (uint32_t l = 0; l < layers; ++l) {
            const uint64_t meta_base =
                static_cast<uint64_t>(l) *
            ...
            for (uint32_t b = included_begin; b < included_end; ++b) {
                ...
                if (count < 0) {
                    return scorer_unavailable(
                        failure_reason,
                        "gpu_adaptive_tensor_gemm_negative_block_count");
                }
        ...
        const uint64_t requested_bytes = std::max<uint64_t>(
            block_store_->config().index_staging_bytes,
            max_block_prototypes * row_bytes);
        const uint64_t capacity_rows =
            std::max<uint64_t>(1, requested_bytes / row_bytes);
        ...
        const uint64_t block_stats_entries =
            local_distributions * block_count;
        const uint64_t block_stats_bytes =
            block_stats_entries * 2ull * sizeof(float);
        const uint64_t workspace_cap =
            static_cast<uint64_t>(env_uint32_or(
                "QW3_KVMEM_ADAPTIVE_BLOCK_STATS_MIB", 512)) *
            1024ull * 1024ull;
        const uint64_t gemm_logits_entries =
            static_cast<uint64_t>(n_tokens) * capacity_rows * gqa_group;
        if (capacity_rows <= UINT32_MAX &&
            block_stats_entries > 0 &&
            block_stats_bytes <= workspace_cap &&
            gemm_logits_entries > 0) {
            const uint64_t distributions64 =
                static_cast<uint64_t>(layers) * n_tokens * cfg.n_heads;
            if (distributions64 == 0 || distributions64 > UINT32_MAX) {
                return scorer_unavailable(
                    failure_reason,
                    "gpu_adaptive_tensor_gemm_distribution_overflow");
            }
            const uint32_t distributions =
                static_cast<uint32_t>(distributions64);
            if (!g_kbar_stream_global_max_ ||
                ...
                g_kbar_stream_global_max_ = backend_.tensor_f32(
                    distributions,
                    "g_adaptive_gpu_global_max");
                g_kbar_stream_global_sum_ = backend_.tensor_f32(
                    distributions,
                    "g_adaptive_gpu_global_sum");
                g_kbar_stream_distribution_capacity_ = distributions;
            }
            ...
            if (!g_adaptive_stream_block_max_ ||
                ...
                g_adaptive_stream_block_stats_capacity_ <
                    block_stats_entries) {
                g_adaptive_stream_block_max_ = backend_.tensor_f32(
                    block_stats_entries,
                    "g_adaptive_gpu_block_max");
                g_adaptive_stream_block_sum_ = backend_.tensor_f32(
                    block_stats_entries,
                    "g_adaptive_gpu_block_sum");
                g_adaptive_stream_block_stats_capacity_ =
                    block_stats_entries;
            }
            if (!g_adaptive_stream_logits_ ||
                g_adaptive_stream_logits_capacity_ <
                    gemm_logits_entries) {
                g_adaptive_stream_logits_ = backend_.scratch_f32(
                    gemm_logits_entries,
                    "g_adaptive_gpu_gemm_logits");
                g_adaptive_stream_logits_capacity_ =
                    gemm_logits_entries;
            }
            const uint64_t query_head_major_elems =
                static_cast<uint64_t>(cfg.n_heads) * n_tokens *
                cfg.head_dim;
            if (!g_adaptive_query_head_major_ ||
                g_adaptive_query_head_major_capacity_ <
                    query_head_major_elems) {
                g_adaptive_query_head_major_ = backend_.tensor_f16(
                    query_head_major_elems,
                    "g_adaptive_query_head_major");
                g_adaptive_query_head_major_capacity_ =
                    query_head_major_elems;
            }
            ...
            for (uint32_t l = 0; l < layers; ++l) {
                AdaptiveGpuLayerArena &arena =
                    g_adaptive_gpu_layer_arenas_[l];
                require_status(
                    backend_.adaptive_pack_query_head_major_device(
                        *g_adaptive_query_head_major_, *g_query_multi_,
                        l, n_tokens, q_layer_stride,
                ...
                while (first < included_end) {
                    ...
                    if (begin_i32 < 0) {
                        return scorer_unavailable(
                            failure_reason,
                            "gpu_adaptive_tensor_gemm_negative_offset");
                    }
                    const uint32_t prototype_begin =
                        static_cast<uint32_t>(begin_i32);
                    uint32_t last = first;
                    ...
                    while (last < included_end) {
                        ...
                        if (offset_i32 < 0 || count_i32 < 0) {
                            return scorer_unavailable(
                                failure_reason,
                                "gpu_adaptive_tensor_gemm_invalid_metadata");
                        }
                        const uint64_t proposed_end =
                            static_cast<uint64_t>(
                                static_cast<uint32_t>(offset_i32)) +
                    ...
                    if (last == first ||
                        ...
                        return scorer_unavailable(
                            failure_reason,
                            "gpu_adaptive_tensor_gemm_tile_oob");
                    }
                    ...
                    if (!st.ok) {
                        return scorer_backend_unavailable(
                            failure_reason,
                            "gpu_adaptive_head_major_gemm_score_failed", st);
                    }
                    copied_bytes +=
                ...
                const DeviceStatus st =
                    backend_.block_attn_stream_adaptive_finalize_device(
                        ...
                        *g_kbar_stream_global_max_,
                        *g_kbar_stream_global_sum_,
                        l, layers, n_tokens, block_count,
                        block_count, included_begin, cfg.n_heads);
                if (!st.ok) {
                    return scorer_backend_unavailable(
                        failure_reason,
                        "gpu_adaptive_tensor_gemm_finalize_failed", st);
                }
            }
            require_status(backend_.synchronize());
            if (std::getenv("QW3_KVMEM_TRACE") ||
                ...
                std::fprintf(
                    ...
                    "layout=per-layer-incremental mode=tiled-tensor-gemm "
                    "exact=1 layers=%u blocks=%u query_tokens=%u "
                    "gqa_group=%u tiles=%u d2d_gib=%.3f "
                    "total_ms=%.3f\n",
                    layers, n_blocks, n_tokens, gqa_group, tiles,
    ...
    for (uint32_t l = 0; l < layers; ++l) {
        ...
        if (begin_i32 < 0 || last_offset_i32 < 0 ||
            ...
            return scorer_unavailable(
                failure_reason,
                "gpu_adaptive_layer_negative_metadata");
        }
        const uint64_t prototype_begin =
            static_cast<uint32_t>(begin_i32);
        const uint64_t prototype_end =
            static_cast<uint64_t>(
                static_cast<uint32_t>(last_offset_i32)) +
            static_cast<uint32_t>(last_count_i32);
        if (!arena.values || !arena.block_offsets ||
            !arena.block_counts ||
            arena.metadata_capacity < block_count ||
            prototype_end < prototype_begin ||
            prototype_end > arena.prototype_count ||
            prototype_end - prototype_begin > UINT32_MAX) {
            return scorer_unavailable(
                failure_reason,
                "gpu_adaptive_layer_metadata_oob");
        }
        ...
        const DeviceStatus st =
            backend_.block_attn_score_adaptive_layer_device(
                *g_score_dev_, *g_query_multi_,
                *arena.values, *arena.block_offsets,
                *arena.block_counts, l, layers, n_tokens,
                q_layer_stride,
        ...
        if (!st.ok) {
            return scorer_backend_unavailable(
                failure_reason,
                "gpu_adaptive_layer_scorer_failed", st);
        }
    }
    if (std::getenv("QW3_KVMEM_TRACE") ||
        kvmem_perf_trace_flag()) {
        std::fprintf(
            stderr,
            "[bs-adaptive-score] placement=gpu "
            "layout=per-layer-incremental mode=layer-one-pass "
            "layers=%u blocks=%u query_tokens=%u "
            "gqa_group=%u submit_ms=%.3f\n",
            layers, n_blocks, n_tokens,
            cfg.n_kv_heads > 0
                ? cfg.n_heads / cfg.n_kv_heads
                : 0,
            (kvmem_steady_ns() - begin_ns) / 1.0e6);
...
bool QwenExecutor::kvmem_score_adaptive_index(
    ...
    if (!env_flag_enabled(
            "QW3_KVMEM_ADAPTIVE_GPU_PACKED", false)) {
        return kvmem_score_gpu_adaptive_index_layer_one_pass(
            n_blocks, n_tokens, q_layer_stride, scale,
            excl_lo_end, excl_hi_begin, accumulate,
            failure_reason);
    }
...
bool QwenExecutor::kvmem_score_host_query_chunks(
    ...
    uint32_t chunk_tokens = configured_chunk;
    // A host-backed long query normally uses bounded 256-token GPU staging.
    ...
    const bool adaptive_exact_tiled =
        ...
        env_flag_enabled("QW3_KVMEM_ADAPTIVE_GPU_TENSOR_GEMM", true);
    if (!score_chunk_env && kvmem_adaptive_prototypes() &&
        adaptive_exact_tiled && block_store_ &&
        block_store_->config().adaptive_score_mode ==
            KvMemAdaptiveScoreMode::Auto) {
        const uint32_t included_begin =
            std::min(excl_lo_end, n_blocks);
        ...
        const uint64_t bytes_per_token =
            included_blocks * cfg.n_heads * 2ull * sizeof(float);
        const uint64_t workspace_cap =
            static_cast<uint64_t>(env_uint32_or(
                "QW3_KVMEM_ADAPTIVE_BLOCK_STATS_MIB", 512)) *
            1024ull * 1024ull;
    ...
    for (uint32_t token_base = 0; token_base < tokens;) {
        ...
        if (kvmem_adaptive_prototypes()) {
            ...
            streamed_ok = kvmem_score_cpu_mean_index(
                n_blocks, count, /*q_layer_stride=*/count,
                scale, excl_lo_end, excl_hi_begin,
                round_mode, chunks == 0 ? 0u : 1u,
                failure_reason);
        } else if (round_mode) {
            score_status = backend_.block_attn_score_softmax_groups_device(
                *kvmem_group_score_dev_, *g_query_multi_, *g_kbar_multi_,
                *kvmem_round_group_begin_dev_,
                *kvmem_round_group_end_dev_,
                static_cast<uint32_t>(
                    kvmem_retrieval_group_spans_.size()),
            ...
            score_status = backend_.block_attn_score_softmax_pages_device(
...
bool QwenExecutor::kvmem_retrieval_score_mean_softmax(
    ...
    if (round_mode) {
        ...
        kvmem_round_group_scores_.clear();
        if (!kvmem_prepare_round_score_groups(nb)) {
            return scorer_unavailable(
    ...
    const uint32_t output_count = round_mode
        ? static_cast<uint32_t>(kvmem_retrieval_group_spans_.size())
        : nb;
    std::vector<float> score(output_count, 0.0f);
    if (auto cp = backend_.copy_to_host(
            round_mode ? *kvmem_group_score_dev_ : *g_score_dev_,
            score.data(), 0, output_count);
        !cp.ok) {
        return scorer_backend_unavailable(
    ...
    std::vector<double> best(block_store_->block_count(), 0.0);
    if (round_mode) {
        if (block_store_->config().group_score_reduce ==
            KvMemGroupScoreReduce::LengthNormalizedMass) {
            const double alpha =
                block_store_->config().group_length_norm_alpha;
            for (uint32_t g = 0;
                 g < score.size() &&
                 g < kvmem_round_group_begin_host_.size() &&
                 g < kvmem_round_group_end_host_.size(); ++g) {
                const int32_t begin =
                    kvmem_round_group_begin_host_[g];
                const int32_t end =
                    kvmem_round_group_end_host_[g];
                const uint32_t slices =
                    end > begin
                        ? static_cast<uint32_t>(end - begin)
                        : 1u;
                score[g] = static_cast<float>(
                    static_cast<double>(score[g]) /
                    std::pow(static_cast<double>(slices), alpha));
            }
        }
        kvmem_round_group_scores_.assign(score.begin(), score.end());
        const uint32_t bt =
            std::max<uint32_t>(1, block_store_->config().block_tokens);
        // Project the logical score back onto physical blocks for diagnostics
        // and safe fallback only. Round selection consumes the group vector
        // directly; this projection is not used to rank individual blocks.
        for (uint32_t g = 0;
             g < kvmem_retrieval_group_spans_.size(); ++g) {
            const auto &[begin, end] =
                kvmem_retrieval_group_spans_[g];
            if (end <= begin) continue;
            const uint32_t first = begin / bt;
            const uint32_t last = (end - 1) / bt;
            for (uint32_t id = first;
                 id <= last && id < best.size(); ++id) {
                best[id] = std::max(best[id],
                                    static_cast<double>(score[g]));
            }
        }
    } else {
    ...
    block_store_->set_retrieval_scores(best);
    if (!g_adaptive_partial_selection_.empty()) {
        const std::vector<uint32_t> final_selection =
            kvmem_selection_with_pin();
        std::unordered_set<uint32_t> partial_ids(
            g_adaptive_partial_selection_.begin(),
            g_adaptive_partial_selection_.end());
        uint32_t intersection = 0;
        for (uint32_t id : final_selection) {
            intersection += partial_ids.count(id) ? 1u : 0u;
        }
        std::fprintf(
            stderr,
            "[bs-adaptive-partial] layers=%u candidates=%zu final=%zu "
            "intersection=%u recall=%.6f\n",
            std::min<uint32_t>(
                env_uint32_or("QW3_KVMEM_ADAPTIVE_PARTIAL_LAYERS", 0), L),
            g_adaptive_partial_selection_.size(), final_selection.size(),
            intersection,
            final_selection.empty()
                ? 1.0
                : static_cast<double>(intersection) /
                      final_selection.size());
        g_adaptive_partial_selection_.clear();
    }
    if (perf_trace) {
        std::fprintf(
