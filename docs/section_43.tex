...
critical path.  In our controlled CPU-only experiment, aggregate stage-out
decreased from \(2{,}786\)~ms to \(50\)~ms.  On the matched 512K-context
sample, aggregate stage-out decreased from \(4{,}579\)~ms to \(39\)~ms,
turning pressure-time eviction into an almost metadata-only operation.
...
In our controlled CPU stage-in experiment, packed transfer reduced stage-in
from \(1{,}583\)~ms to \(359\)~ms and reduced H2D submission time from
\(1{,}518\)~ms to \(43.5\)~ms.  The full rematerialization pipeline reduced
final KV assembly and re-RoPE from \(2{,}032\)~ms to \(687\)~ms, a
\(2.96\times\) speedup.
...
overhead decreased from approximately \(7.21\)~s to \(1.03\)~s, an
\(85.7\%\) reduction.  Lower-level mechanisms such as scalable one-dot
